...
extends GenericNameValueRecordingFrame {
    ...
    private final Row rootRow = new Row();
    ...
    private final List<String> segmentFilePaths = new ArrayList<String>();
    ...
    public void logTimeSpan(GameProfiler.ProfileArea area) {
        ...
        Span newSpan = this.allocSpan(area);
        Row parentRow = this.rootRow;
    ...
    protected Span allocSpan(GameProfiler.ProfileArea area) {
    ...
    private void writeDataRow(StringBuilder logLine, Row row) {
        int columnCount = row.spans.size();
    ...
    private void writeFrameTimeRow(StringBuilder logLine, Row row, int segmentNo) {
        GameProfileRecording.appendCell(logLine, row.startTime / 100L);
        GameProfileRecording.appendCell(logLine, row.endTime / 100L);
        GameProfileRecording.appendCell(logLine, segmentNo);
    }

    private void writeSpan(StringBuilder logLine, Row parentRow, Span span) {
        long startTime = (span.startTime - parentRow.startTime) / 100L;
        long length = (span.endTime - span.startTime) / 100L;
        GameProfileRecording.appendCell(logLine, span.key);
        GameProfileRecording.appendCell(logLine, span.depth);
        GameProfileRecording.appendCell(logLine, startTime);
        GameProfileRecording.appendCell(logLine, length);
        int spanCount = span.children.size();
        for (int i = 0; i < spanCount; ++i) {
            Span childSpan = span.children.get(i);
    ...
    protected void writeHeaderToMemory() {
        ...
        for (Map.Entry<String, Integer> entry : this.keyValueTable.entrySet()) {
            line.setLength(0);
