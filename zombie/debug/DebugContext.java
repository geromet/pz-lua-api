...
public class DebugContext {
    ...
    public void tick() {
        ...
        for (i = 0; i < this.windows.size(); ++i) {
            ...
            if (!window.open.get()) continue;
            window.endFrameTick();
        }
        for (i = 0; i < this.transientWindows.size(); ++i) {
            window = this.transientWindows.get(i);
            if (!window.open.get()) continue;
            window.endFrameTick();
        }
    }
    ...
    private void doMainMenu() {
        if (ImGui.beginMainMenuBar()) {
            if (ImGui.beginMenu("Windows")) {
                ...
                for (i = 0; i < this.windows.size(); ++i) {
                    window = this.windows.get(i);
                    if (!ImGui.menuItem(window.getTitle(), null, window.open.get())) continue;
    ...
    public void tickFrameStart() {
        ...
        this.dockspace = !Core.isUseGameViewport() ? ImGui.dockSpaceOverViewport(ImGui.getMainViewport(), 8) : ImGui.dockSpaceOverViewport(ImGui.getMainViewport());
        for (i = 0; i < this.windows.size(); ++i) {
            window = this.windows.get(i);
            if (!window.open.get()) continue;
    ...
    public BaseDebugWindow getExistingTransientWindow(BaseDebugWindow window) {
        for (int i = 0; i < this.transientWindows.size(); ++i) {
            ...
            if (!transientWindow.getTitle().equals(window.getTitle())) continue;
