        ...
        if (this.doTreeNode("gunplayTargets", "Targets", false, false)) {
            if (ImGui.beginTable("gunplayTargetTable", 10, 1984)) {
                ...
                for (int i = 0; i < hitInfoList.size(); ++i) {
                    ...
                    ImGui.tableSetColumnIndex(columnIndex++);
                    ImGui.text(Float.toString(PZMath.max(0.0f, this.combatConfig.get(CombatConfigKey.LOW_LIGHT_TO_HIT_MAXIMUM_PENALTY) * (1.0f - obj.getCurrentSquare().getLightLevel(player.getPlayerNum()) / this.combatConfig.get(CombatConfigKey.LOW_LIGHT_THRESHOLD)))));
                    ImGui.tableSetColumnIndex(columnIndex++);
