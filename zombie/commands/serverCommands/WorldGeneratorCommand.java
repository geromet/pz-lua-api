...
public class WorldGeneratorCommand
extends CommandBase {
    ...
    protected String Command() {
        if (this.getCommandArgsCount() >= 1) {
            ...
            WorldGenerate.instance.getStatus().threadCount = this.getCommandArgsCount() == 2 && "slow".equals(this.getCommandArg(1)) ? 1 : Runtime.getRuntime().availableProcessors();
            if ("start".equals(application)) {
                WorldGenerate.instance.init(Core.gameSaveWorld);
                return "Preparing for generating map: " + WorldGenerate.instance.getStatus().generatedChunks + "/" + WorldGenerate.instance.getStatus().allChunks + " using " + WorldGenerate.instance.getStatus().threadCount + " threads";
            }
            ...
            if ("status".equals(application)) {
                return "Generating map: " + WorldGenerate.instance.getStatus().generatedChunks + "/" + WorldGenerate.instance.getStatus().allChunks + " using " + WorldGenerate.instance.getStatus().threadCount + " threads";
            }
        }
        return this.getHelp();
    }
