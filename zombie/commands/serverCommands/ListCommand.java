...
extends CommandBase {
    ...
    public static String List(String subsystem) {
        ...
        if ("animals".equalsIgnoreCase(subsystem)) {
            ...
            for (IsoAnimal animal : AnimalInstanceManager.getInstance().getAnimals()) {
                ...
                result.append("* ").append(animal.getOnlineID()).append(" ").append(animal.getAnimalType()).append(" ").append(animal.getBreed().getName()).append(" ").append(animal.getHutch() == null ? "world" : "hutch").append("\n");
            }
        } else {
    ...
    protected String Command() {
        return ListCommand.List(this.getCommandArg(0));
    }
}
