...
@AltCommandArgs(value={@CommandArgs(required={"(.+)", "(-?\\d+.*\\d*),(-?\\d+.*\\d*),(-?\\d+.*\\d*)"}, argName="Teleport user"), @CommandArgs(required={"(-?\\d+.*\\d*),(-?\\d+.*\\d*),(-?\\d+.*\\d*)"}, argName="teleport me")})
...
extends CommandBase {
    ...
    private String TeleportMeToCoords() {
        float x = this.coords[0].floatValue();
        float y = this.coords[1].floatValue();
        float z = this.coords[2].floatValue();
        ...
        LoggerManager.getLogger("admin").write(this.getExecutorUsername() + " teleported to " + PZMath.fastfloor(x) + "," + PZMath.fastfloor(y) + "," + PZMath.fastfloor(z));
        return "teleported to " + PZMath.fastfloor(x) + "," + PZMath.fastfloor(y) + "," + PZMath.fastfloor(z) + " please wait two seconds to show the map around you.";
    }
    ...
    private String TeleportUserToCoords() {
        ...
        float y = this.coords[1].floatValue();
        float z = this.coords[2].floatValue();
        if (this.connection != null && !this.connection.getRole().hasCapability(Capability.TeleportPlayerToAnotherPlayer) && !this.username.equals(this.getExecutorUsername())) {
            return "An Observer can only teleport himself";
        }
        ...
        LoggerManager.getLogger("admin").write(this.getExecutorUsername() + " teleported to " + PZMath.fastfloor(x) + "," + PZMath.fastfloor(y) + "," + PZMath.fastfloor(z));
        return this.username + " teleported to " + PZMath.fastfloor(x) + "," + PZMath.fastfloor(y) + "," + PZMath.fastfloor(z) + " please wait two seconds to show the map around you.";
    }
