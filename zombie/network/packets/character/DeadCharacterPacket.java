...
implements INetworkPacket {
    ...
    public void processClient(UdpConnection connection) {
        ...
        if (this.characterId.getCharacter().getCurrentSquare() != square) {
            DebugLog.Multiplayer.debugln(String.format("Corpse %s(%d) teleport: position (%f ; %f) => (%f ; %f)", this.characterId.getCharacter().getClass().getSimpleName(), this.characterId.getID(), Float.valueOf(this.characterId.getCharacter().getX()), Float.valueOf(this.characterId.getCharacter().getY()), Float.valueOf(this.x), Float.valueOf(this.y)));
        ...
        if (this.characterId.getCharacter().getAnimAngleRadians() - this.angle > 1.0E-4f) {
            DebugLog.Multiplayer.debugln(String.format("Corpse %s(%d) teleport: direction (%f) => (%f)", this.characterId.getCharacter().getClass().getSimpleName(), this.characterId.getID(), Float.valueOf(this.characterId.getCharacter().getAnimAngleRadians()), Float.valueOf(this.angle)));
            if (this.characterId.getCharacter().hasAnimationPlayer() && this.characterId.getCharacter().getAnimationPlayer().isReady() && !this.characterId.getCharacter().getAnimationPlayer().isBoneTransformsNeedFirstFrame()) {
        ...
        if (this.isFallOnFront != this.characterId.getCharacter().isFallOnFront()) {
            DebugLog.Multiplayer.debugln(String.format("Corpse %s(%d) teleport: pose (%s) => (%s)", this.characterId.getCharacter().getClass().getSimpleName(), this.characterId.getID(), this.characterId.getCharacter().isFallOnFront() ? "front" : "back", this.isFallOnFront ? "front" : "back"));
