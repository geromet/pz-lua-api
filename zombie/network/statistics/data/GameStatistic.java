...
implements IStatistic {
    ...
    public final Counter animalObjects = new Counter(this, "animals-objects", 0.0, () -> IsoWorld.instance.currentCell.getObjectList().stream().filter(obj -> obj instanceof IsoAnimal).count(), "Animals in world", "number");
    public final Counter animalInstances = new Counter(this, "animals-instances", 0.0, () -> AnimalInstanceManager.getInstance().getAnimals().size(), "Animals to sync", "number");
    ...
    public final Counter zombiesLoaded = new Counter(this, "zombies-loaded", 0.0, () -> IsoWorld.instance.getCell().getZombieList().size(), "Zombies loaded", "number");
    public final Counter zombiesSimulated = new Counter(this, "zombies-simulated", 0.0, () -> (long)(this.zombiesUpdated.get() / Math.max((double)MovingObjectUpdateScheduler.instance.getFrameCounter() - this.zombiesUpdates.get(), 1.0)), "Zombies simulated", "number");
    ...
    public int getPlayersCount() {
        ...
        if (GameClient.client) {
            return GameClient.IDToPlayerMap.size();
        }
        return 0;
    }
    ...
    public int getZombiesCount() {
        ...
        if (GameClient.client) {
            return GameClient.IDToZombieMap.size();
        }
        return 0;
    }
}
