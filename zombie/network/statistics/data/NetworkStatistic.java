...
implements IStatistic {
    ...
    private void updateConnection(UdpConnection connection) {
        long period = (long)ServerOptions.getInstance().multiplayerStatisticsPeriod.getValue() * 1000L;
        ...
        this.voipReceived.set(this.voipReceived.get() / (double)period);
        this.voipSent.set(this.voipSent.get() / (double)period);
    }
    ...
    public void addIncomePacket(short id, int size, UdpConnection connection) {
        ...
        if (packetType != null) {
            StatisticManager.getInstance().observeServerPacketProcessDuration(packetType.name(), connection.getUserName() == null ? "" : connection.getUserName(), size);
    ...
    public void addOutcomePacket(short id, int size, UdpConnection connection) {
        PacketTypes.PacketType packetType = PacketTypes.packetTypes.get(id);
        if (packetType != null) {
            StatisticManager.getInstance().observeServerPacketSendDuration(packetType.name(), connection.getUserName() == null ? "" : connection.getUserName(), size);
