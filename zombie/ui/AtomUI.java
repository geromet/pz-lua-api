...
implements UIElementInterface {
    ...
    public void render() {
        ...
        if (this.stencil) {
            ...
            for (int i = 0; i < this.nodes.size(); ++i) {
                this.nodes.get((int)i).stencilNode = this;
                this.nodes.get(i).render();
            }
            this.clearStencilRect();
        } else {
            for (int i = 0; i < this.nodes.size(); ++i) {
                this.nodes.get((int)i).stencilNode = this.stencilNode;
                this.nodes.get(i).render();
            }
        }
    }
    ...
    public void setWidthSilent(double value) {
        this.width = value;
        this.table.rawset("width", (Object)value);
        this.updateInternalValues();
    }
    ...
    public void setHeightSilent(double value) {
        this.height = value;
        this.table.rawset("height", (Object)value);
        this.updateInternalValues();
    }
    ...
    public boolean isOverElement(double mx, double my) {
        ...
        return this.isOverElementLocal(mx - this.x, my - this.y);
    }
    ...
    public boolean onConsumeMouseButtonDown(int btn, double x, double y) {
        ...
        for (int i = this.nodes.size() - 1; i >= 0; --i) {
            ...
            if (!consumed && ui.isOverElementLocal(lx - ui.x, ly - ui.y)) {
                if (!ui.onConsumeMouseButtonDown(btn, lx - ui.x, ly - ui.y)) continue;
                consumed = true;
                continue;
            }
        ...
        if (!consumed && lx >= this.leftSide && ly >= this.topSide && lx < this.rightSide && ly < this.downSide && this.luaMouseButtonDown != null) {
            Boolean res = LuaManager.caller.protectedCallBoolean(UIManager.getDefaultThread(), this.luaMouseButtonDown, new Object[]{this.table, BoxedStaticValues.toDouble(btn), BoxedStaticValues.toDouble(lx), BoxedStaticValues.toDouble(ly)});
    ...
    public boolean onConsumeMouseButtonUp(int btn, double x, double y) {
        ...
        for (int i = this.nodes.size() - 1; i >= 0; --i) {
            AtomUI ui = this.nodes.get(i);
            if (!consumed && ui.isOverElementLocal(lx - ui.x, ly - ui.y)) {
                if (!ui.onConsumeMouseButtonUp(btn, lx - ui.x, ly - ui.y)) continue;
                consumed = true;
                continue;
            }
        ...
        if (!consumed && lx >= this.leftSide && ly >= this.topSide && lx < this.rightSide && ly < this.downSide && this.luaMouseButtonUp != null) {
            Boolean res = LuaManager.caller.protectedCallBoolean(UIManager.getDefaultThread(), this.luaMouseButtonUp, new Object[]{this.table, BoxedStaticValues.toDouble(btn), BoxedStaticValues.toDouble(lx), BoxedStaticValues.toDouble(ly)});
            return res == Boolean.TRUE;
        }
        return consumed;
    }
    ...
    public void onMouseButtonDownOutside(int btn, double x, double y) {
        ...
        if (this.luaMouseButtonDownOutside != null) {
            LuaManager.caller.pcallvoid(UIManager.getDefaultThread(), this.luaMouseButtonDownOutside, new Object[]{this.table, BoxedStaticValues.toDouble(btn), BoxedStaticValues.toDouble(local[0]), BoxedStaticValues.toDouble(local[1])});
        }
    }
    ...
    public void onMouseButtonUpOutside(int btn, double x, double y) {
        ...
        if (this.luaMouseButtonUpOutside != null) {
            LuaManager.caller.pcallvoid(UIManager.getDefaultThread(), this.luaMouseButtonUpOutside, new Object[]{this.table, BoxedStaticValues.toDouble(btn), BoxedStaticValues.toDouble(local[0]), BoxedStaticValues.toDouble(local[1])});
        }
    }
    ...
    public Boolean onConsumeMouseWheel(double del, double x, double y) {
        ...
        for (int i = this.nodes.size() - 1; i >= 0 && !consumed; --i) {
            AtomUI ui = this.nodes.get(i);
            if (!ui.isOverElementLocal(lx - ui.x, ly - ui.y) || !ui.onConsumeMouseWheel(del, lx - ui.x, ly - ui.y).booleanValue()) continue;
            consumed = true;
        }
        if (!consumed && lx >= this.leftSide && ly >= this.topSide && lx < this.rightSide && ly < this.downSide && this.luaMouseWheel != null) {
            Boolean res = LuaManager.caller.protectedCallBoolean(UIManager.getDefaultThread(), this.luaMouseWheel, new Object[]{this.table, BoxedStaticValues.toDouble(del), BoxedStaticValues.toDouble(lx), BoxedStaticValues.toDouble(ly)});
            return res == Boolean.TRUE;
        }
        return consumed;
    }
    ...
    public Boolean onConsumeMouseMove(double dx, double dy, double x, double y) {
        ...
        for (int i = this.nodes.size() - 1; i >= 0; --i) {
            AtomUI ui = this.nodes.get(i);
            if (!consumed && ui.isOverElementLocal(lx - ui.x, ly - ui.y)) {
                if (!ui.onConsumeMouseMove(dx, dy, lx - ui.x, ly - ui.y).booleanValue()) continue;
                consumed = true;
                continue;
            }
        ...
        if (!consumed && lx >= this.leftSide && ly >= this.topSide && lx < this.rightSide && ly < this.downSide && this.luaMouseMove != null) {
            Boolean res = LuaManager.caller.protectedCallBoolean(UIManager.getDefaultThread(), this.luaMouseMove, new Object[]{this.table, BoxedStaticValues.toDouble(dx), BoxedStaticValues.toDouble(dy)});
            return res == Boolean.TRUE;
        }
        return consumed;
    }
    ...
    public void onExtendMouseMoveOutside(double dx, double dy, double x, double y) {
        ...
        if (this.luaMouseMoveOutside != null) {
            LuaManager.caller.pcallvoid(UIManager.getDefaultThread(), this.luaMouseMoveOutside, new Object[]{this.table, BoxedStaticValues.toDouble(dx), BoxedStaticValues.toDouble(dy)});
        }
    }
    ...
    public boolean onConsumeKeyPress(int key) {
        ...
        for (int i = this.nodes.size() - 1; i >= 0; --i) {
            AtomUI ui = this.nodes.get(i);
            if (!ui.onConsumeKeyPress(key)) continue;
            return true;
        }
    ...
    public boolean onConsumeKeyRepeat(int key) {
        ...
        for (int i = this.nodes.size() - 1; i >= 0; --i) {
            AtomUI ui = this.nodes.get(i);
            if (!ui.onConsumeKeyRepeat(key)) continue;
            return true;
        }
    ...
    public boolean onConsumeKeyRelease(int key) {
        ...
        for (int i = this.nodes.size() - 1; i >= 0; --i) {
            AtomUI ui = this.nodes.get(i);
            if (!ui.onConsumeKeyRelease(key)) continue;
            return true;
        }
    ...
    public KahluaTable getLuaLocalPosition(double x, double y) {
        ...
        data.rawset("x", (Object)pos[0]);
        data.rawset("y", (Object)pos[1]);
        return data;
    }
    ...
    public KahluaTable getLuaParentPosition(double x, double y) {
        double xx = (x *= this.scaleX) * this.cosA + (y *= this.scaleY) * this.sinA;
        double yy = -x * this.sinA + y * this.cosA;
        KahluaTable data = LuaManager.platform.newTable();
        data.rawset("x", (Object)xx);
        data.rawset("y", (Object)yy);
        return data;
    }
    ...
    public KahluaTable getColor() {
        KahluaTable data = LuaManager.platform.newTable();
        data.rawset("r", (Object)BoxedStaticValues.toDouble(this.colorR));
        data.rawset("g", (Object)BoxedStaticValues.toDouble(this.colorG));
        data.rawset("b", (Object)BoxedStaticValues.toDouble(this.colorB));
        data.rawset("a", (Object)BoxedStaticValues.toDouble(this.colorA));
        return data;
    }
    ...
    void loadFromTable() {
        ...
        Object object = UIManager.tableget(this.table, "anchorLeft");
        this.anchorLeft = object instanceof Double ? (left = (Double)object) : null;
        object = UIManager.tableget(this.table, "anchorRight");
        this.anchorRight = object instanceof Double ? (right = (Double)object) : null;
        object = UIManager.tableget(this.table, "anchorTop");
        this.anchorTop = object instanceof Double ? (top = (Double)object) : null;
        object = UIManager.tableget(this.table, "anchorDown");
        this.anchorDown = object instanceof Double ? (down = (Double)object) : null;
    ...
    double[] getAbsolutePosition(double localX, double localY) {
        double parentLocalX = (localX *= this.scaleX) * this.cosA + (localY *= this.scaleY) * this.sinA + this.x;
        double parentLocalY = -localX * this.sinA + localY * this.cosA + this.y;
        if (this.parentNode != null) {
            return this.parentNode.getAbsolutePosition(parentLocalX, parentLocalY);
        }
        return new double[]{parentLocalX, parentLocalY};
    }
    ...
    boolean isOverElementLocal(double x, double y) {
        ...
        for (int i = 0; i < this.nodes.size(); ++i) {
            AtomUI ui = this.nodes.get(i);
            if (!ui.isOverElementLocal(localX - ui.x, localY - ui.y)) continue;
            return true;
        }
        return false;
    }
    ...
    public void setStencilRect() {
        ...
        SpriteRenderer.instance.render(null, leftTop[0], leftTop[1], rightTop[0], rightTop[1], rightDown[0], rightDown[1], leftDown[0], leftDown[1], 1.0f, 0.0f, 0.0f, 1.0f, null);
    ...
    double tryGetDouble(String key, double defaultValue) {
        double d;
        Object object = UIManager.tableget(this.table, key);
    ...
    LuaClosure tryGetClosure(String key) {
        ...
        Object object = UIManager.tableget(this.table, key);
        return object instanceof LuaClosure ? (luaClosure = (LuaClosure)object) : null;
    }

    String tryGetString(String key, String defaultValue) {
        String s;
        Object object = UIManager.tableget(this.table, key);
        return object instanceof String ? (s = (String)object) : defaultValue;
    }
}
