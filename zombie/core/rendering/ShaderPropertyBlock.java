...
public class ShaderPropertyBlock {
    ...
    public void SetShader(Shader shader) {
        ...
        for (Map.Entry<String, ShaderParameter> kvp : this.parameters.entrySet()) {
        ...
        if (shader == null || !shader.isInstanced()) {
            ...
            for (Map.Entry<String, ShaderParameter> kvp : bufferData.parameters.entrySet()) {
    ...
    public void CopyParameters(ShaderPropertyBlock from) {
        for (Map.Entry<String, ShaderParameter> kvp : from.parameters.entrySet()) {
    ...
    public Matrix4f SetMatrix4(String name, FloatBuffer value) {
        ...
        if (param1 == null) {
            ...
            matrix = param1.GetType() != ShaderParameter.ParameterTypes.Matrix4 ? new Matrix4f() : param1.GetMatrix4();
            matrix.load(value);
    ...
    private <T> void SetArrayElement(String name, int index, T value, Class<T[]> token) {
        ShaderParameter param = this.parameters.getOrDefault(name, null);
        if (param != null) {
            T[] array = token.cast(param.GetValue());
            array[index] = value;
        }
    }

    public void SetFloatArrayElement(String name, int index, float value) {
        this.SetArrayElement(name, index, Float.valueOf(value), Float[].class);
    }
    ...
    public void SetVector2ArrayElement(String name, int index, float x, float y) {
        ShaderParameter param = this.parameters.getOrDefault(name, null);
        if (param != null) {
            Vector2f[] array = (Vector2f[])Vector2f[].class.cast(param.GetValue());
            array[index].set(x, y);
        }
    }
    ...
    public void SetVector3ArrayElement(String name, int index, float x, float y, float z) {
        ShaderParameter param = this.parameters.getOrDefault(name, null);
        if (param != null) {
            Vector3f[] array = (Vector3f[])Vector3f[].class.cast(param.GetValue());
    ...
    public void SetVector4ArrayElement(String name, int index, float x, float y, float z, float w) {
        ShaderParameter param = this.parameters.getOrDefault(name, null);
        if (param != null) {
            Vector4f[] array = (Vector4f[])Vector4f[].class.cast(param.GetValue());
    ...
    private <T> T[] CreateAndFillArray(Class<T[]> arrayToken, Class<T> token, int count) {
        T[] array = arrayToken.cast(Array.newInstance(token, count));
        try {
            Constructor<T> constructor = token.getDeclaredConstructor(new Class[0]);
            for (int i = 0; i < count; ++i) {
                array[i] = token.cast(constructor.newInstance(new Object[0]));
            }
        }
