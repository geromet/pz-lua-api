...
public final class VBORendererCommands {
    ...
    public void adopt(VBORendererCommands rhs) {
        ...
        this.commandCount += rhs.commandCount;
    ...
    public int position() {
        ...
        return this.commandBuffer.position();
    }
    ...
    private void reserve(int numBytes) {
        if (this.commandBuffer == null) {
            int blocks = (int)PZMath.ceil((float)numBytes / 512.0f);
            this.commandBuffer = ByteBuffer.allocateDirect(blocks * 512);
            return;
        }
        if (this.commandBuffer.position() + numBytes <= this.commandBuffer.capacity()) {
            return;
        }
        int blocks = (int)PZMath.ceil((float)(this.commandBuffer.position() + numBytes) / 512.0f);
