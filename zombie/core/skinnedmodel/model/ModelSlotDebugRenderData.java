...
extends PooledObject {
    ...
    public void initModel(ModelSlotRenderData slotData) {
        for (int i = 0; i < slotData.modelData.size(); ++i) {
            ModelInstanceRenderData instData = (ModelInstanceRenderData)slotData.modelData.get(i);
    ...
    private void initSkeleton() {
        ...
        if (this.slotData.object instanceof BaseVehicle) {
            for (int i = 0; i < this.slotData.modelData.size(); ++i) {
                ModelInstanceRenderData instData = (ModelInstanceRenderData)this.slotData.modelData.get(i);
                ModelInstance modelInstance = instData.modelInstance;
                if (!(modelInstance instanceof VehicleSubModelInstance)) continue;
                VehicleSubModelInstance vsmi = (VehicleSubModelInstance)modelInstance;
    ...
    private void initWeaponHitPoint() {
        ...
        InventoryItem inventoryItem = this.slotData.character.getPrimaryHandItem();
        if (!(inventoryItem instanceof HandWeapon)) {
            return;
        }
        HandWeapon weapon = (HandWeapon)inventoryItem;
        this.weaponLength = weapon.weaponLength;
        Matrix4f boneMtx = this.slotData.animPlayer.getModelTransformAt(boneIndex);
        this.weaponMatrix = (org.joml.Matrix4f)BaseVehicle.TL_matrix4f_pool.get().alloc();
        PZMath.convertMatrix(boneMtx, this.weaponMatrix);
