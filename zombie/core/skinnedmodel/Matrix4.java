...
public class Matrix4 {
    ...
    public Matrix4 clearToOrtho(float left, float right, float bottom, float top, float near, float far) {
        return this.clear().put(0, 2.0f / (right - left)).put(5, 2.0f / (top - bottom)).put(10, -2.0f / (far - near)).put(12, -(right + left) / (right - left)).put(13, -(top + bottom) / (top - bottom)).put(14, -(far + near) / (far - near)).put(15, 1.0f);
    }
    ...
    public Matrix4 clearToPerspective(float fovRad, float width, float height, float near, float far) {
        ...
        return this.clear().put(0, fov / (width / height)).put(5, fov).put(10, (far + near) / (near - far)).put(14, 2.0f * far * near / (near - far)).put(11, -1.0f);
    }
