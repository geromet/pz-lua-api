...
public class TransactionManager {
    ...
    public static void update() {
        if (GameServer.server) {
            ...
            transactionForDelete.forEach(transaction -> {
                ...
                DebugLog.Objects.noise("Timeout: %s", transaction);
            });
        }
    }
    ...
    public static byte isConsistent(int itemId, InventoryItem worldItem, ItemContainer source2, ItemContainer destination, String extra, ItemTransactionPacket dropTransaction, IsoPlayer player) {
        ...
        if (destination == null && (dropTransaction == null || dropTransaction.sourceId.containerType != ContainerID.ContainerType.IsoObject || dropTransaction.destinationId.containerType != ContainerID.ContainerType.IsoObject)) {
        ...
        for (Transaction transaction : transactions) {
            ...
            if (itemId != -1) {
                if (destination != null && destination.getCharacter() != null) {
                    if (itemId == transaction.itemId && extra == null || source2 != null && source2.id == transaction.itemId || transaction.sourceId.getContainer() == null || itemId == transaction.sourceId.getContainer().id || itemId == transaction.destinationId.getContainer().id) {
                        DebugLog.Objects.noise("Inconsistent: item to inventory (%s) t=(%s)", description, transaction);
                        return 1;
                    }
                } else if (destination != null && (itemId == transaction.itemId || destination.id == transaction.itemId && transaction.itemId != -1 || transaction.sourceId.isContainerTheSame(itemId, destination) && transaction.itemId == -1)) {
                    DebugLog.Objects.noise("Inconsistent: item from inventory (%s) t=(%s)", description, transaction);
                    return 1;
                }
            } else if (destination.getCharacter() != null && source2 != null && (transaction.destinationId.isContainerTheSame(itemId, source2) || transaction.sourceId.isContainerTheSame(itemId, source2))) {
                DebugLog.Objects.noise("Inconsistent: object to inventory (%s) t=(%s)", description, transaction);
                return 1;
            }
            if (transaction.itemId == -1 || transaction.sourceId.getContainer() == null || transaction.destinationId.getContainer() == null || !transaction.destinationId.isContainerTheSame(itemId, destination) || !transaction.sourceId.getContainer().containsID(transaction.itemId) || (item = transaction.sourceId.getContainer().getItemWithID(transaction.itemId)) == null) continue;
    ...
    public static void cancelAllRelevantToUser(IsoPlayer player) {
        for (Transaction transaction : transactions) {
            if ((transaction.sourceId.containerType != ContainerID.ContainerType.PlayerInventory || transaction.sourceId.playerId.getID() != player.onlineId) && (transaction.destinationId.containerType != ContainerID.ContainerType.PlayerInventory || transaction.destinationId.playerId.getID() != player.onlineId)) continue;
    ...
    public static byte createItemTransaction(IsoPlayer player, InventoryItem item, ItemContainer source2, ItemContainer destination) {
        return TransactionManager.createItemTransaction(player, item, source2, destination, IsoDirections.N, 0.0f, 0.0f, 0.0f);
    }

    public static byte createItemTransaction(IsoPlayer player, InventoryItem item, ItemContainer source2, ItemContainer destination, IsoDirections direction, float posX, float posY, float posZ) {
        int itemId = -1;
        if (item != null) {
            itemId = item.id;
        }
        ...
        if (TransactionManager.isConsistent(itemId, item, source2, destination, null, null, player) == 0) {
            ItemTransactionPacket transaction = new ItemTransactionPacket();
            try {
                transaction.set(player, item, source2, destination, null, direction, posX, posY, posZ);
                ...
                DebugLog.General.printException(e, "createItemTransaction: error while creating package. player=" + String.valueOf(player) + " item=" + String.valueOf(item) + " source=" + String.valueOf(source2) + " destination=" + String.valueOf(destination) + " direction=" + String.valueOf((Object)direction) + " posX=" + posX + " posY=" + posY + " posZ=" + posZ, LogSeverity.Error);
                return 0;
            }
    ...
    public static byte changeItemTypeTransaction(IsoPlayer player, InventoryItem item, ItemContainer source2, ItemContainer destination, String extra) {
        if (TransactionManager.isConsistent(item.id, item, source2, destination, extra, null, player) == 0) {
            ItemTransactionPacket transaction = new ItemTransactionPacket();
            transaction.set(player, item, source2, destination, extra, null, 0.0f, 0.0f, 0.0f);
            ByteBufferWriter bbw2 = GameClient.connection.startPacket();
            PacketTypes.PacketType.ItemTransaction.doPacket(bbw2);
            transaction.write(bbw2);
            PacketTypes.PacketType.ItemTransaction.send(GameClient.connection);
