...
import zombie.core.textures.APNGFrame;
...
public final class PNGDecoder {
     ...
     */
    public void decode(ByteBuffer buffer, int stride, int inHeight, Format fmt, int inChunkType) throws IOException {
    ...
    public void decodeFlipped(ByteBuffer buffer, int stride, int inHeight, Format fmt, int inChunkType) throws IOException {
        if (stride <= 0) {
    ...
    public void decodeFrame(MipMapLevel compositeBuffer, ImageDataFrame frame, ByteBuffer buffer, int stride, Format fmt) throws IOException {
        int offsetDst;
        int offsetSrc;
        int x;
        int y;
        ...
        switch (frame.apngFrame.blendOp) {
            ...
            case 1: {
                for (y = 0; y < frame.apngFrame.height; ++y) {
                    for (x = 0; x < frame.apngFrame.width; ++x) {
                        offsetSrc = (frame.apngFrame.xOffset + x) * 4 + (frame.apngFrame.yOffset + y) * stride;
                        offsetDst = (frame.apngFrame.xOffset + x) * 4 + (frame.apngFrame.yOffset + y) * compositeStride;
                        int rSrc = buffer.get(offsetSrc) & 0xFF;
                        int gSrc = buffer.get(offsetSrc + 1) & 0xFF;
                        int bSrc = buffer.get(offsetSrc + 2) & 0xFF;
                        float aSrc = (float)(buffer.get(offsetSrc + 3) & 0xFF) / 255.0f;
                        int rDst = compositeBB.get(offsetDst) & 0xFF;
                        int gDst = compositeBB.get(offsetDst + 1) & 0xFF;
                        int bDst = compositeBB.get(offsetDst + 2) & 0xFF;
                        float aDst = (float)(compositeBB.get(offsetDst + 3) & 0xFF) / 255.0f;
                        buffer.put(offsetSrc, (byte)((float)rSrc * aSrc + (float)rDst * (1.0f - aSrc)));
                        buffer.put(offsetSrc + 1, (byte)((float)gSrc * aSrc + (float)gDst * (1.0f - aSrc)));
                        buffer.put(offsetSrc + 2, (byte)((float)bSrc * aSrc + (float)bDst * (1.0f - aSrc)));
                        buffer.put(offsetSrc + 3, aSrc > 0.0f || aDst > 0.0f ? (byte)-1 : 0);
                    }
                }
                break;
            }
    ...
    private int readfdAT() throws IOException {
        ...
        int sequenceNumber = this.readInt(this.buffer, 0);
