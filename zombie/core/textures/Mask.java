...
Cloneable {
    ...
    protected Mask(Texture texture, WrappedBuffer wb) {
        ...
        int ys = (int)(texture.getYStart() * (float)th);
        int ye = (int)(texture.getYEnd() * (float)th);
        this.mask = new BooleanGrid(this.width, this.height);
        texture.setMask(this);
        ByteBuffer temp = wb.getBuffer();
        temp.rewind();
        block0: for (int y = 0; y < texture.getHeightHW(); ++y) {
    ...
    public Mask(ITexture texture, boolean[] mask) {
        ...
        int xs = (int)(texture.getXStart() * (float)t);
        int xe = (int)(texture.getXEnd() * (float)t);
        t = texture.getHeightHW();
        int ys = (int)(texture.getYStart() * (float)t);
    ...
    public boolean get(int x, int y) {
        ...
        if (this.isSubMask()) {
            ...
            return this.mask.getValue(this.offsetX + x, this.offsetY + y);
        }
