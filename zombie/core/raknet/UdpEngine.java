    ...
    public void connected() {
        ...
        if (GameClient.client) {
            if (!(GameClient.askPing || GameClient.sendQR || GameClient.askCustomizationData)) {
                ...
                INetworkPacket.send(PacketTypes.PacketType.ServerCustomization, new Object[]{GameClient.serverName, ServerCustomizationPacket.Data.ServerImageIcon});
                INetworkPacket.send(PacketTypes.PacketType.ServerCustomization, new Object[]{GameClient.serverName, ServerCustomizationPacket.Data.ServerImageLoginScreen});
                INetworkPacket.send(PacketTypes.PacketType.ServerCustomization, new Object[]{GameClient.serverName, ServerCustomizationPacket.Data.ServerImageLoadingScreen});
                INetworkPacket.send(PacketTypes.PacketType.ServerCustomization, new Object[]{GameClient.serverName, ServerCustomizationPacket.Data.Done});
            }
        }
    }
    ...
    private void decode(ByteBufferReader buf) {
        ...
        switch (packetIdentifier) {
            ...
            case 46: {
                ...
                DebugLog.Voice.println("RakVoice channel is closed \"%s\" guid=%d", connection.getUserName(), connection.getConnectedGUID());
                break;
            }
            case 16: {
                int id = buf.getByte() & 0xFF;
            ...
            case 134: {
                ...
                if (GameServer.server) {
                    ...
                    if (con == null) {
                        DebugLog.Network.warn("GOT PACKET FROM UNKNOWN CONNECTION guid=%d packetId=%d", guid, userPacketId);
                        return;
                    }
            ...
            default: {
                ...
                long guid = this.peer.getGuidOfPacket();
                UdpConnection connection = this.connectionMap.get(guid);
                AntiCheat.PacketRakNet.act(connection, String.valueOf(packetIdentifier));
            }
        }
    }
