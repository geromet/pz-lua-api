...
import zombie.iso.worldgen.zombie.ZombieVoronoiReader;
...
public class ZombieVoronoi {
    ...
    public final Map<CellCoord, double[]> cellCache = new HashMap<CellCoord, double[]>();
    ...
    public static List<ZombieVoronoi> getVoronois(long seed) {
        ...
        ArrayList<ZombieVoronoi> voronois = new ArrayList<ZombieVoronoi>();
        for (int i = 0; i < entries.size(); ++i) {
            voronois.add(new ZombieVoronoi(seed + (long)i, entries.get(i)));
        }
        return voronois;
    }
        ...
        for (int xOffset = -1; xOffset <= 1; ++xOffset) {
            ...
            for (int yOffset = -1; yOffset <= 1; ++yOffset) {
                ...
                for (int i = 0; i < this.numberPoints; ++i) {
                    points.add(new Coord(rng.nextDouble() + (double)secX, rng.nextDouble() + (double)secY));
                }
            }
        }
    ...
    public double[] evaluateCellNoise(int wx, int wy) {
        Map<CellCoord, double[]> map = this.cellCache;
        synchronized (map) {
        ...
        for (int x = 0; x < 32; ++x) {
            for (int y = 0; y < 32; ++y) {
                chunkNoise[x + y * 32] = this.evaluateNoise((double)(x + wx * 32) / this.scale, (double)(y + wy * 32) / this.scale, rng, points);
            }
        }
        Map<CellCoord, double[]> map2 = this.cellCache;
        synchronized (map2) {
    ...
    public void releaseCell(int cellX, int cellY) {
        Map<CellCoord, double[]> map = this.cellCache;
        synchronized (map) {
            double[] dArray = this.cellCache.remove(new CellCoord(cellX, cellY));
        }
    }
}
