...
public class OreVein {
    ...
    public OreVein(int cellX, int cellY, OreVeinConfig config, Random rnd) {
        ...
        float deltaAngle = 360.0f / (float)this.armsAmount;
        for (int i = 0; i < this.armsAmount; ++i) {
            float rao = (rnd.nextFloat() * 2.0f - 1.0f) * (float)config.getArmsDeltaAngle();
            this.armsOrientation[i] = deltaAngle * (float)i + offsetAngle + rao;
            this.armsDistance[i] = rnd.nextFloat() * (float)config.getArmsDistMax() + (float)config.getArmsDistMin();
            this.endPoints[i] = new Vector2(this.startPoint.x + (float)(Math.sin(Math.toRadians(this.armsOrientation[i])) * (double)this.armsDistance[i]), this.startPoint.y + (float)(-Math.cos(Math.toRadians(this.armsOrientation[i])) * (double)this.armsDistance[i]));
        }
    ...
    public boolean isValid(int x, int y, Random rnd) {
        Vector2 d2 = new Vector2(this.startPoint.x - (float)x, this.startPoint.y - (float)y);
        float lengthSquared = d2.getLengthSquared();
        if (lengthSquared < (float)(this.centerRadius * this.centerRadius) && rnd.nextFloat() < this.centerProb) {
            return true;
        }
        for (int i = 0; i < this.armsAmount; ++i) {
            if (lengthSquared > this.armsDistance[i] * this.armsDistance[i]) continue;
            Vector2 d1p = new Vector2(this.endPoints[i].x - this.startPoint.x, this.endPoints[i].y - this.startPoint.y);
            Vector2 d3p = new Vector2(this.endPoints[i].x - (float)x, this.endPoints[i].y - (float)y);
            d1p.normalize();
            d3p.normalize();
            if (!(Math.abs(d1p.x - d3p.x) < 0.001f) || !(Math.abs(d1p.y - d3p.y) < 0.001f) || !(rnd.nextFloat() < this.armsProb)) continue;
            return true;
        }
        return false;
    }
    ...
    public String toString() {
        return "OreVein{startPoint=" + String.valueOf(this.startPoint) + ", amountArms=" + this.armsAmount + ", orientationArms=" + Arrays.toString(this.armsOrientation) + ", endPoints=" + Arrays.toString(this.endPoints) + ", distanceArms=" + Arrays.toString(this.armsDistance) + ", singleFeatures=" + String.valueOf(this.tileGroups) + "}";
    }
}
