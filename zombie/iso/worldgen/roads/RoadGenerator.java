...
public class RoadGenerator {
    ...
    private List<Vector2D> genRoadDelaunayPoints() {
        HashSet<Vector2D> nexus = new HashSet<Vector2D>();
        for (int x = IsoWorld.instance.metaGrid.minX; x <= IsoWorld.instance.metaGrid.maxX; ++x) {
            for (int y = IsoWorld.instance.metaGrid.minY; y <= IsoWorld.instance.metaGrid.maxY; ++y) {
                ...
                if (!((double)rand.nextFloat() < this.config.probability())) continue;
    ...
    private Map<ChunkCoord, Set<RoadEdge>> getRoadEdgesMap(List<RoadNexus> roadNexus) {
        HashMap<ChunkCoord, Set<RoadEdge>> roadEdgesMap = new HashMap<ChunkCoord, Set<RoadEdge>>();
        List<RoadEdge> roadEdges = this.getRoadEdges(roadNexus);
        for (RoadEdge roadEdge : roadEdges) {
            ChunkCoord coordA = new ChunkCoord(PZMath.fastfloor((float)roadEdge.a.x / 8.0f), PZMath.fastfloor((float)roadEdge.a.y / 8.0f));
            ChunkCoord coordB = new ChunkCoord(PZMath.fastfloor((float)roadEdge.b.x / 8.0f), PZMath.fastfloor((float)roadEdge.b.y / 8.0f));
            ChunkCoord coordSub = new ChunkCoord(PZMath.fastfloor((float)roadEdge.subnexus.x / 8.0f), PZMath.fastfloor((float)roadEdge.subnexus.y / 8.0f));
            HashSet<RoadEdge> roadEdgeA = (HashSet<RoadEdge>)roadEdgesMap.get(coordA);
            HashSet<RoadEdge> roadEdgeB = (HashSet<RoadEdge>)roadEdgesMap.get(coordB);
            HashSet<RoadEdge> roadEdgeSub = (HashSet<RoadEdge>)roadEdgesMap.get(coordSub);
            if (roadEdgeA == null) {
                roadEdgeA = new HashSet<RoadEdge>();
            }
            if (roadEdgeB == null) {
                roadEdgeB = new HashSet<RoadEdge>();
            }
            if (roadEdgeSub == null) {
                roadEdgeSub = new HashSet<RoadEdge>();
            }
    ...
    public Set<Road> getRoads(int cx, int cy) {
        ...
        for (int x = IsoWorld.instance.metaGrid.minX * 32; x <= IsoWorld.instance.metaGrid.maxX * 32; ++x) {
            edges = this.roadEdges.get(new ChunkCoord(x, cy));
            if (edges == null) continue;
            for (RoadEdge re : edges) {
                for (Road road : re.roads) {
                    if (road.getDirection() != RoadDirection.WE || Math.min(road.getCA().x(), road.getCB().x()) > cx || Math.max(road.getCA().x(), road.getCB().x()) < cx) continue;
                    roads.add(road);
                }
            }
        }
        for (int y = IsoWorld.instance.metaGrid.minY * 32; y <= IsoWorld.instance.metaGrid.maxY * 32; ++y) {
            edges = this.roadEdges.get(new ChunkCoord(cx, y));
            if (edges == null) continue;
            for (RoadEdge re : edges) {
                for (Road road : re.roads) {
                    if (road.getDirection() != RoadDirection.NS || Math.min(road.getCA().y(), road.getCB().y()) > cy || Math.max(road.getCA().y(), road.getCB().y()) < cy) continue;
                    roads.add(road);
                }
            }
        }
