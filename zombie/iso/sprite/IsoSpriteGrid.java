...
public final class IsoSpriteGrid {
    ...
    public void setSprite(int x, int y, int z, IsoSprite sprite) {
        if (this.isValidXYZ(x, y, z)) {
            this.sprites[this.getSpriteIndex((int)x, (int)y, (int)z)] = sprite;
        }
    }
    ...
    public int getSpriteIndex(IsoSprite sprite) {
        for (int i = 0; i < this.sprites.length; ++i) {
            ...
            if (sprite1 == null || sprite1 != sprite) continue;
            return i;
        }
        return -1;
    }
    ...
    public int getSpriteGridPosX(IsoSprite sprite) {
        ...
        if (index >= 0) {
            return (index %= this.getWidth() * this.getHeight()) % this.getWidth();
        }
        return -1;
    }
    ...
    public int getSpriteGridPosZ(IsoSprite sprite) {
        int index = this.getSpriteIndex(sprite);
        if (index >= 0) {
            return index / (this.getWidth() * this.getHeight());
        }
        return -1;
    }

    public IsoSprite getSpriteFromIndex(int index) {
        if (index >= 0 && index < this.getSpriteCount()) {
    ...
    public boolean validate() {
        ...
        for (int z = 0; z < this.getLevels(); ++z) {
            ...
            for (int i = 0; i < this.getWidth() * this.getHeight(); ++i) {
                if (this.sprites[levelStartIndex + i] == null) continue;
                bLevelEmpty = false;
                break;
            }
            if (!bLevelEmpty) continue;
            return false;
        }
        return true;
    }
