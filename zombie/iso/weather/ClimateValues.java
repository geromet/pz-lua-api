...
public class ClimateValues {
    ...
    public void pollDate(GregorianCalendar calendar) {
        ...
        double pollWorldAgeHours = currentWorldAgeHours + (difference /= 60.0);
        float pollTime = (float)calendar.get(11) + (float)calendar.get(12) / 60.0f;
    ...
    protected void updateValues(double worldAgeHours, float time, ClimateManager.DayInfo currentDay, ClimateManager.DayInfo nextDay) {
        ...
        if (currentDay.year != this.cacheYear || currentDay.month != this.cacheMonth || currentDay.day != this.cacheDay) {
            ...
            seed += (long)(currentDay.month * currentDay.day * 1234);
            seed += (long)((currentDay.year - 1990) * currentDay.month * 10000);
            this.seededRandom.setSeed(seed += (long)((baseSeedB - baseSeedA) * currentDay.day));
            this.dayFogStrength = 0.0f;
            this.dayDoFog = false;
            this.dayFogDuration = 0.0f;
            float r = this.seededRandom.nextInt(1000);
            boolean bl = this.dayDoFog = r < 200.0f;
            if (this.dayDoFog) {
                ...
                float airmassSample = (float)SimplexNoise.noise(this.simplexOffsetA, (worldAgeHours + 12.0 - 48.0) / this.clim.getAirMassNoiseFrequencyMod(SandboxOptions.instance.getRainModifier()));
        ...
        boolean belowMean = curTemperature < (curMeanTemperature = ClimateManager.clerp(dayLerpVal, currentDay.season.getDayMeanTemperature(), nextDay.season.getDayMeanTemperature()));
        ...
        this.temperature = this.baseTemperature + 1.0f - (tempMod += 5.0f * this.nightLagged);
        this.temperatureIsSnow = this.temperature < 0.0f || ClimateManager.winterIsComing;
        ...
        this.windIntensity = windBase *= 0.65f;
