...
public class DesignationZone {
    ...
    public DesignationZone(String type, String name, int x, int y, int z, int x2, int y2, boolean doSync) {
        ...
        this.w = x2 - x;
        this.h = y2 - y;
        if (DesignationZone.getZoneById(this.id) == null) {
            allZones.add(this);
            if (doSync) {
                this.sync();
            }
    ...
    public static void removeZone(String type, String name) {
        DesignationZone zone = DesignationZone.getZoneByNameAndType(type, name);
        if (zone != null) {
    ...
    public static DesignationZone getZoneByName(String name) {
        for (int i = 0; i < allZones.size(); ++i) {
            ...
            if (!zone.name.equals(name)) continue;
            return zone;
        }
        return null;
    }
    ...
    public static DesignationZone getZoneByNameAndType(String type, String name) {
        for (int i = 0; i < allZones.size(); ++i) {
            DesignationZone zone = allZones.get(i);
            if (!zone.name.equals(name) || !zone.type.equals(type)) continue;
            return zone;
        }
        return null;
    }
    ...
    public static DesignationZone getZoneByType(String type, int x, int y, int z) {
        for (int i = 0; i < allZones.size(); ++i) {
            DesignationZone zone = allZones.get(i);
            if (x < zone.x || x >= zone.x + zone.w || y < zone.y || y >= zone.y + zone.h || zone.z != z || !type.equals(zone.type)) continue;
            return zone;
        }
        return null;
    }
    ...
    public static DesignationZone getZoneById(Double id) {
        for (int i = 0; i < allZones.size(); ++i) {
            if (!DesignationZone.allZones.get((int)i).id.equals(id)) continue;
    ...
    public IsoGridSquare getRandomFreeSquare() {
        for (int i = 0; i < 1000; ++i) {
            IsoGridSquare sq = IsoWorld.instance.getCell().getGridSquare(Rand.Next(this.x + 1, this.x + this.w - 1), Rand.Next(this.y + 1, this.y + this.h - 1), this.z);
            if (!sq.isFree(true)) continue;
            return sq;
        }
        return IsoWorld.instance.getCell().getGridSquare(Rand.Next(this.x + 1, this.x - this.w - 1), Rand.Next(this.y + 1, this.y - this.h - 1), this.z);
    }
    ...
    public static ArrayList<DesignationZone> getAllZonesByType(String type) {
        ArrayList<DesignationZone> result = new ArrayList<DesignationZone>();
        for (int i = 0; i < allZones.size(); ++i) {
            DesignationZone zone = allZones.get(i);
            if (!type.equals(zone.type)) continue;
            result.add(zone);
        }
        return result;
    }
