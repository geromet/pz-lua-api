...
public final class IsoRoomExit {
    ...
    public IsoObject getDoor(IsoCell cell) {
        ...
        if (sq != null) {
            ...
            if (!sq.getSpecialObjects().isEmpty() && sq.getSpecialObjects().get(0) instanceof IsoThumpable && ((IsoThumpable)sq.getSpecialObjects().get(0)).isDoor()) {
                return sq.getSpecialObjects().get(0);
            }
        }
        if ((sq = cell.getGridSquare(this.x, this.y + 1, this.layer)) != null) {
            if (!sq.getSpecialObjects().isEmpty() && sq.getSpecialObjects().get(0) instanceof IsoDoor) {
                return sq.getSpecialObjects().get(0);
            }
            if (!sq.getSpecialObjects().isEmpty() && sq.getSpecialObjects().get(0) instanceof IsoThumpable && ((IsoThumpable)sq.getSpecialObjects().get(0)).isDoor()) {
        ...
        if ((sq = cell.getGridSquare(this.x + 1, this.y, this.layer)) != null) {
            if (!sq.getSpecialObjects().isEmpty() && sq.getSpecialObjects().get(0) instanceof IsoDoor) {
                return sq.getSpecialObjects().get(0);
            }
            if (!sq.getSpecialObjects().isEmpty() && sq.getSpecialObjects().get(0) instanceof IsoThumpable && ((IsoThumpable)sq.getSpecialObjects().get(0)).isDoor()) {
