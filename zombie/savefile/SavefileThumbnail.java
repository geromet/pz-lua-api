...
public final class SavefileThumbnail {
    ...
    static final CopyTextureDrawer copyTextureDrawer;
    ...
    public static void create() {
        ...
        SavefileThumbnail.create(playerIndex);
    }
    ...
    public static void create(int playerIndex) {
        ...
        SavefileThumbnail.setZoom(playerIndex, 1.0f, 1.0f);
        IsoCamera.cameras[playerIndex].center();
        creatingThumbnail = true;
        try {
            SavefileThumbnail.renderWorld(playerIndex, true, true);
            SpriteRenderer.instance.drawGeneric(new TakeScreenShotDrawer(playerIndex));
        }
        finally {
            creatingThumbnail = false;
        }
        SavefileThumbnail.setZoom(playerIndex, oldZoom, oldTargetZoom);
        IsoCamera.cameras[playerIndex].center();
        SavefileThumbnail.restoreScreenImage();
    ...
    private static void renderWorld(int playerIndex, boolean bClear, boolean bFixCamera) {
        ...
        SavefileThumbnail.StartFrame(playerIndex, bClear);
        if (bFixCamera) {
            SpriteRenderer.instance.drawGeneric(new FixCameraDrawer(playerIndex));
        }
    ...
    private static void StartFrame(int nPlayer, boolean clear) {
        ...
        SavefileThumbnail.copyScreenImage();
        Texture.bindCount = 0;
    ...
    private static void createWithRenderShader(int playerIndex) {
        ...
        try {
            ...
            float u0 = (float)sx / (float)offscreenW;
            float u1 = (float)(sx + 256) / (float)offscreenW;
            float v0 = (float)sy / (float)offscreenH;
            float v1 = (float)(sy + 256) / (float)offscreenH;
    ...
    static {
        copyTextureDrawer = new CopyTextureDrawer();
    }
    ...
    extends TextureDraw.GenericDrawer {
        ...
        TakeScreenShotDrawer(int playerIndex) {
            ...
            Shader shader = SceneShaderStore.weatherShader;
            if (shader instanceof WeatherShader) {
                WeatherShader weatherShader = (WeatherShader)shader;
    ...
    static final class CopyTextureDrawer
    extends TextureDraw.GenericDrawer {
        Texture copyTexture;
        TextureFBO copyFbo;
        ...
        void copyTexture(TextureFBO fboSrc, TextureFBO fboDst) {
            ...
            float u0 = 0.0f;
            float v0 = 0.0f;
            float u1 = (float)texSrc.getWidth() / (float)texSrc.getWidthHW();
            float v1 = (float)texSrc.getHeight() / (float)texSrc.getHeightHW();
