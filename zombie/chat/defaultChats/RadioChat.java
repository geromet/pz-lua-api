...
extends RangeBasedChat {
    public RadioChat(ByteBufferReader bb, ChatTab tab, IsoPlayer owner) {
        ...
        if (!this.isCustomSettings()) {
            this.setSettings(RadioChat.getDefaultSettings());
        }
    ...
    protected void showInSpeechBubble(ChatMessage msg) {
        ...
        this.getSpeechBubble().addChatLine(msg.getText(), color.r, color.g, color.b, UIFont.Dialogue, this.getRange(), this.customTag, this.isAllowBBcode(), this.isAllowImages(), this.isAllowChatIcons(), this.isAllowColors(), this.isAllowFonts(), this.isEqualizeLineHeights());
    }
    ...
    private String getRadioChannelStr(ChatMessage msg) {
        ...
        for (hz = channel % 1000; hz % 10 == 0 && hz != 0; hz /= 10) {
        }
