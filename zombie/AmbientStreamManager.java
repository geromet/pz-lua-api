...
extends BaseAmbientStreamManager {
    ...
    public void addRandomAmbient(boolean force) {
        ...
        if (!GameClient.client) {
            DebugLog.Sound.debugln("playing ambient: " + sound + " at dist: " + Math.abs(x - player.getX()) + "," + Math.abs(y - player.getY()));
    ...
    public void doAlarm(RoomDef room) {
        if (room != null && room.building != null && room.building.alarmed) {
            ...
            DebugLog.Sound.debugln("nights survived = " + (float)(GameTime.getInstance().getWorldAgeHours() / 24.0 + (double)((SandboxOptions.instance.timeSinceApo.getValue() - 1) * 30)));
