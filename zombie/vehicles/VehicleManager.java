...
public final class VehicleManager {
        ...
        if (vehicle.vehicleId != -1) {
            ...
            if (GameServer.server) {
                for (int j = 0; j < GameServer.udpEngine.connections.size(); ++j) {
                    ...
                    if (vehicle.connectionState[connection.getIndex()] == null) continue;
        ...
        for (i = 0; i < this.connected.length; ++i) {
            ...
            if (this.connected[i] != null && !GameServer.udpEngine.connections.contains(this.connected[i])) {
                ...
                for (j = 0; j < vehicles.size(); ++j) {
                    vehicles.get((int)j).connectionState[i] = null;
                }
                this.connected[i] = null;
                continue;
            }
            for (j = 0; j < vehicles.size(); ++j) {
                if (vehicles.get((int)j).connectionState[i] == null) continue;
                vehicles.get((int)j).connectionState[i].flags = (short)(vehicles.get((int)j).connectionState[i].flags | vehicles.get((int)j).updateFlags);
            }
        }
    ...
    private void sendVehicles(UdpConnection connection) {
        ...
        for (BaseVehicle vehicle : IsoWorld.instance.currentCell.getVehicles()) {
            ...
            if (!(state = vehicle.connectionState[connection.getIndex()]).shouldSend(vehicle)) continue;
            if ((state.flags & 1) != 0) {
    ...
    public void clientUpdate() {
        ...
        for (int i = 0; i < vehicles.size(); ++i) {
            ...
            if (vehicle.interpolation.interpolationDataGet(dd = this.tempFloats, this.engineSound)) {
                if (vehicle.isNetPlayerAuthorization(BaseVehicle.Authorization.Local) || vehicle.isNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide)) continue;
