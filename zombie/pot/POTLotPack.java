...
public final class POTLotPack {
    ...
    void loadChunk(int chunkX, int chunkY) throws IOException {
        ...
        in.seek((long)((this.version >= 1 ? 8 : 0) + 4) + (long)index * 8L);
        ...
        for (int z = minZ; z <= maxZ; ++z) {
            for (int x = 0; x < this.chunkDim; ++x) {
                for (int y = 0; y < this.chunkDim; ++y) {
                    ...
                    this.offsetInData[squareXYZ += lwx * this.chunkDim + lwy * this.chunkDim * this.cellDim + (z - this.lotHeader.minLevel) * this.cellDim * this.cellDim] = -1;
                    if (skip > 0) {
                        --skip;
                        continue;
                    }
        ...
        for (int z = this.lotHeader.minLevelNotEmpty; z <= this.lotHeader.maxLevelNotEmpty; ++z) {
            for (int x = 0; x < this.chunkDim; ++x) {
                for (int y = 0; y < this.chunkDim; ++y) {
                    ...
                    int offset = this.offsetInData[squareXYZ += chunkX * this.chunkDim + chunkY * this.chunkDim * this.cellDim];
                    if (offset == -1) {
                        ++notdonecount;
                        continue;
                    }
    ...
    String[] getSquareData(int squareX, int squareY, int z) {
        int squareXYZ = (squareX -= this.lotHeader.getMinSquareX()) + (squareY -= this.lotHeader.getMinSquareY()) * this.cellDim + (z - this.lotHeader.minLevel) * this.cellDim * this.cellDim;
