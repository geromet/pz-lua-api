...
import zombie.characters.animals.pathfind.LowLevelGoalNode;
...
import zombie.characters.animals.pathfind.RobustLineIntersector;
import zombie.core.math.PZMath;
import zombie.popman.ObjectPool;

public final class LowLevelAStar
extends AStar {
    ...
    void findPath(float x1, float y1, int z1, float x2, float y2, int z2, ArrayList<Vector2f> pathOut) {
        ...
        if (mesh1 == mesh2 && startTri == (endTri |= mesh2.indexOf() << 16)) {
            if (pathOut != null) {
        ...
        initialNode.meshIdx = startTri >> 16 & 0xFFFF;
        initialNode.triangleIdx = startTri & 0xFFFF;
        ...
        goalNode1.meshIdx = endTri >> 16 & 0xFFFF;
        goalNode1.triangleIdx = endTri & 0xFFFF;
        goalNode1.edgeIdx = -1;
        goalNode1.x = x2;
        goalNode1.y = y2;
        ...
        if (path != null) {
            ...
            if (betterPath != null) {
                ...
                AnimalPathfind.getInstance().vector3fObjectPool.releaseAll((List<Vector3f>)betterPath);
            }
        }
        LowLevelSearchNode.pool.release(initialNode);
        LowLevelSearchNode.pool.release(goalNode1);
        LowLevelSearchNode.pool.releaseAll((List<LowLevelSearchNode>)new ArrayList<LowLevelSearchNode>(this.nodeMap.valueCollection()));
        this.nodeMap.clear();
    }

    public long makeKey(int meshIdx, int tri) {
        return meshIdx << 16 & 0xFFFF0000 | tri & 0xFFFF;
    }
    ...
    public void getSuccessors(LowLevelSearchNode searchNode, ArrayList<ISearchNode> successors) {
        ...
        if (searchNode.meshList == this.goalNode.searchNode.meshList && searchNode.meshIdx == this.goalNode.searchNode.meshIdx && searchNode.triangleIdx == this.goalNode.searchNode.triangleIdx && searchNode != this.goalNode.searchNode) {
        ...
        for (OffMeshConnection omc : mesh1.offMeshConnections) {
            if (omc.triFrom != searchNode.triangleIdx) continue;
            this.addSuccessor(omc.meshTo.meshList, omc.meshTo.indexOf(), omc.triTo, omc.edgeTo, (omc.edge1.x + omc.edge2.x) / 2.0f, (omc.edge1.y + omc.edge2.y) / 2.0f, successors);
        }
    }
    ...
    void findOverlappingEdges(Mesh mesh1, int tri1, int edge1, Mesh mesh2) {
        ...
        block0: for (int j2 = 0; j2 < mesh2.trianglesOnBoundaries.size(); ++j2) {
            short tri2 = mesh2.trianglesOnBoundaries.get(j2);
            short edges2 = mesh2.edgesOnBoundaries.get(j2);
            for (int edge2 = 0; edge2 < 3; ++edge2) {
                Vector2f p4;
                Vector2f p3;
                if ((edges2 & 1 << edge2) == 0 || RobustLineIntersector.computeIntersection(p1, p2, p3 = mesh2.triangles.get(tri2 + edge2), p4 = mesh2.triangles.get(tri2 + (edge2 + 1) % 3), overlap1, overlap2) != 2) continue;
    ...
    public ArrayList<Vector3f> stringPull(ArrayList<ISearchNode> nodes) {
        ...
        for (int i = 1; i < portals.size() / 2; ++i) {
            ...
            if (!(this.triarea2(portalApex, portalLeft, left) >= 0.0f)) continue;
    ...
    ArrayList<Vector3f> getPortalEdges(ArrayList<ISearchNode> nodes) {
        ArrayList<Vector3f> portals = new ArrayList<Vector3f>();
        ...
        for (int i = 0; i < nodes.size() - 2; ++i) {
            ...
            if (node1.meshList.z != node2.meshList.z) {
                ...
                if (Math.abs(node2.getX() - node1.getX()) > Math.abs(node2.getY() - node1.getY())) {
                    portals.add(vector3fPool.alloc().set((float)x2 + 0.0f, (float)y2 + 0.0f, node2.getZ()));
                    portals.add(vector3fPool.alloc().set((float)x2 + 0.0f, (float)y2 + 1.0f, node2.getZ()));
                    continue;
                }
                portals.add(vector3fPool.alloc().set((float)x2 + 0.0f, (float)y2 + 0.0f, node2.getZ()));
                portals.add(vector3fPool.alloc().set((float)x2 + 1.0f, (float)y2 + 0.0f, node2.getZ()));
                continue;
            }
            Mesh mesh1 = node1.meshList.get(node1.meshIdx);
            int edges = this.getSharedEdge(mesh1, node1.triangleIdx, mesh2 = node2.meshList.get(node2.meshIdx), node2.triangleIdx, edge1, edge2, overlap1, overlap2);
            if (edges == -1) {
                return null;
            }
    ...
    int getSharedEdge(Mesh mesh1, int tri1, Mesh mesh2, int tri2, Vector2f edge1, Vector2f edge2, Vector2f overlap1, Vector2f overlap2) {
        ...
        if (RobustLineIntersector.computeIntersection(p3, p1, p5, p6, overlap1, overlap2) == 2) {
            return this.packEdges(2, 1);
        }
        if (RobustLineIntersector.computeIntersection(p3, p1, p6, p4, overlap1, overlap2) == 2) {
            return this.packEdges(2, 2);
        }
        return -1;
    }
}
