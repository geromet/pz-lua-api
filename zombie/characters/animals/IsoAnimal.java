...
implements IAnimalVisual {
    ...
    public void test() {
        ...
        ((AnimationTrack)multiTrack.getTracks().getFirst()).setCurrentTimeValue(Rand.Next(100));
    }
    ...
    public void respondToSound() {
        ...
        if (this.soundSourceTarget != null) {
            ...
            if (this.timeSinceFleeFromSound <= 0.0f || !this.isAnimalMoving()) {
                this.setMoving(false);
                if (GameServer.server) {
                    sound = (WorldSoundManager.WorldSound)this.soundSourceTarget;
        ...
        sound = WorldSoundManager.instance.getSoundAnimal(this);
        ...
        this.timeSinceFleeFromSound = 1000.0f * ((float)sound.radius / 0.5f);
        IsoAnimal.tempVector2.x = this.getX();
        IsoAnimal.tempVector2.y = this.getY();
        IsoAnimal.tempVector2.x -= (float)sound.x;
        IsoAnimal.tempVector2.y -= (float)sound.y;
        int length = 10;
        if (this.isWild()) {
            length = 20;
            if (sound.source instanceof BaseVehicle) {
        ...
        tempVector2.setLength(Math.max((float)length, (float)length - tempVector2.getLength()));
        int goX = tempVector2.floorX();
        int goY = tempVector2.floorY();
        this.changeStress((float)sound.radius / 10.0f);
    ...
    public void drawRope(IsoGameCharacter chr) {
        ...
        LineDrawer.drawLine(sx0, sy0 -= this.getAnimalSize(), sx1, sy1, r, g, b, 1.0f, 2);
    }
    ...
    private void drawRope(IsoGridSquare sq) {
        ...
        LineDrawer.drawLine(sx0 -= this.getAnimalSize(), sy0 -= this.getAnimalSize(), sx1, sy1, r, g, b, 1.0f, 2);
    }
    ...
    public void renderlast() {
        ...
        if (this.data.getAttachedTree() != null && (treeSq = this.getCell().getGridSquare(this.getData().getAttachedTreeX(), this.getData().getAttachedTreeY(), PZMath.fastfloor(this.getZ()))) != null) {
    ...
    private void doDebugString() {
        ...
        for (short id : this.playerAcceptanceList.keySet()) {
            ...
            sb.append("Acceptance: ").append(Math.round(this.playerAcceptanceList.get(id).floatValue())).append("\n");
        }
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        this.setX(this.setLastX(this.setNextX(this.setScriptNextX(input.getFloat() + (float)(IsoWorld.saveoffsetx * 300)))));
        this.setY(this.setLastY(this.setNextY(this.setScriptNextY(input.getFloat() + (float)(IsoWorld.saveoffsety * 300)))));
    ...
    public String getAgeText(boolean cheat, int skillLvl) {
        ...
        if (this.isBaby()) {
            ...
            if (this.getData().getDaysSurvived() > this.adef.minAge * 2) {
                txt = this.isGeriatric() ? Translator.getText("IGUI_Animal_Geriatric") : Translator.getText("IGUI_Animal_Adult");
    ...
    public String getHealthText(boolean cheat, int skillLvl) {
        ...
        String txt = (double)this.getHealth() > 0.8 ? Translator.getText("IGUI_Animal_Healthy") : ((double)this.getHealth() > 0.55 ? Translator.getText("IGUI_Animal_OffColor") : ((double)this.getHealth() > 0.3 ? Translator.getText("IGUI_Animal_Sickly") : Translator.getText("IGUI_Animal_Dying")));
        if (skillLvl < 5) {
    ...
    public String getAppearanceText(boolean cheat) {
        ...
        String hunger = (double)this.getHunger() < 0.3 ? Translator.getText("IGUI_Animal_WellFed") : ((double)this.getHunger() < 0.6 ? Translator.getText("IGUI_Animal_Underfed") : Translator.getText("IGUI_Animal_Starving"));
        if (cheat) {
            dbgTxt = (String)dbgTxt + "," + df.format(this.getThirst()) + ")";
        }
        String thirst = (double)this.getThirst() < 0.3 ? Translator.getText("IGUI_Animal_FullyWatered") : ((double)this.getThirst() < 0.6 ? Translator.getText("IGUI_Animal_Thirsty") : Translator.getText("IGUI_Animal_DyingThirst"));
    ...
    public String getFullName() {
        if (!StringUtils.isNullOrEmpty(this.customName)) {
            Object txt = "";
            if (this.isWild()) {
                txt = (String)txt + " (" + Translator.getText("IGUI_Animal_Wild") + ")";
            }
            return this.customName + (String)txt;
        }
        Object name = Translator.getText("IGUI_AnimalType_" + this.getAnimalType());
        if (this.getData().getBreed() != null) {
            name = Translator.getText("IGUI_Breed_" + this.getData().getBreed().getName()) + " " + (String)name;
        }
        if (this.isWild()) {
            name = (String)name + " (" + Translator.getText("IGUI_Animal_Wild") + ")";
        }
        return name;
    }
