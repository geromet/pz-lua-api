...
public final class VirtualAnimal {
    ...
    String currentZoneAction = "Follow";
    ...
    void load(ByteBuffer input, int worldVersion) throws IOException {
        ...
        this.zone = zoneID.getMostSignificantBits() == 0L && zoneID.getLeastSignificantBits() == 0L ? null : IsoWorld.instance.metaGrid.animalZoneHandler.getZone(zoneID);
        ...
        for (int i = 0; i < count; ++i) {
            IsoObject obj = IsoObject.factoryFromFileInput(IsoWorld.instance.currentCell, input);
    ...
    public boolean isTimeToSleep() {
        ...
        if (!this.sleepPeriodStart.isEmpty() && !this.sleepPeriodEnd.isEmpty() && this.sleepPeriodStart.size() == this.sleepPeriodEnd.size()) {
            for (int i = 0; i < this.sleepPeriodStart.size(); ++i) {
                ...
                if (start >= (end = this.sleepPeriodEnd.get(i).intValue())) continue;
                if (currentHour >= start && currentHour < end) {
                    return true;
                }
                if (currentHour < end || currentHour >= start) continue;
                return true;
            }
        } else {
    ...
    public boolean isTimeToEat() {
        ...
        if (!this.eatPeriodStart.isEmpty() && !this.eatPeriodEnd.isEmpty() && this.eatPeriodStart.size() == this.eatPeriodEnd.size()) {
            for (int i = 0; i < this.eatPeriodStart.size(); ++i) {
                ...
                if (start >= (end = this.eatPeriodEnd.get(i).intValue())) continue;
                if (currentHour >= start && currentHour < end) {
                    return true;
                }
                if (currentHour < end || currentHour >= start) continue;
    ...
    public String getNextSleepPeriod() {
        ...
        if (!this.sleepPeriodStart.isEmpty() && !this.sleepPeriodEnd.isEmpty() && this.sleepPeriodStart.size() == this.sleepPeriodEnd.size()) {
            for (int i = 0; i < this.sleepPeriodStart.size(); ++i) {
                ...
                if (start - currentHour >= period) continue;
    ...
    public String getEndSleepPeriod() {
        ...
        if (!this.sleepPeriodStart.isEmpty() && !this.sleepPeriodEnd.isEmpty() && this.sleepPeriodStart.size() == this.sleepPeriodEnd.size()) {
            for (int i = 0; i < this.sleepPeriodStart.size(); ++i) {
                ...
                if (end <= currentHour || end - currentHour >= period) continue;
    ...
    public IsoAnimal findAnimalById(int animalID) {
        for (int i = 0; i < this.animals.size(); ++i) {
            ...
            if (isoAnimal.getAnimalID() != animalID) continue;
            return isoAnimal;
        }
        return null;
    }
