...
public class PolygonEditor {
    ...
    void setPlane(Vector3f translate, Vector3f rotate, GridPlane gridPlane) {
        ...
        Matrix4f m = this.scene.allocMatrix4f().rotationXYZ(rotate.x * ((float)Math.PI / 180), rotate.y * ((float)Math.PI / 180), rotate.z * ((float)Math.PI / 180));
    ...
    boolean uiToPlane3D(float uiX, float uiY, Vector3f result) {
        boolean hit = false;
        UI3DScene.Ray cameraRay = this.scene.getCameraRay(uiX, (float)this.scene.screenHeight() - uiY, this.scene.allocRay());
