...
public final class BitHeader {
    ...
    private static BitHeaderBase getHeader(HeaderSize type, ByteBuffer buffer, boolean isWrite) {
        if (type == HeaderSize.Byte) {
            BitHeaderByte header = pool_byte.poll();
            if (header == null) {
                header = new BitHeaderByte();
            }
            header.setBuffer(buffer);
            header.setWrite(isWrite);
            return header;
        }
        if (type == HeaderSize.Short) {
            BitHeaderShort header = pool_short.poll();
            if (header == null) {
                header = new BitHeaderShort();
            }
            header.setBuffer(buffer);
            header.setWrite(isWrite);
            return header;
        }
        if (type == HeaderSize.Integer) {
            BitHeaderInt header = pool_int.poll();
            if (header == null) {
                header = new BitHeaderInt();
            }
            header.setBuffer(buffer);
            header.setWrite(isWrite);
            return header;
        }
        if (type == HeaderSize.Long) {
            BitHeaderLong header = pool_long.poll();
            if (header == null) {
                header = new BitHeaderLong();
            }
            header.setBuffer(buffer);
            header.setWrite(isWrite);
            return header;
    ...
    public static BitHeaderWrite allocWrite(HeaderSize size, ByteBuffer buffer) {
        return BitHeader.allocWrite(size, buffer, false);
    }

    public static BitHeaderWrite allocWrite(HeaderSize size, ByteBuffer buffer, boolean allocOnly) {
        BitHeaderBase header = BitHeader.getHeader(size, buffer, true);
        if (!allocOnly) {
            header.create();
        }
        return header;
    ...
    public static BitHeaderRead allocRead(HeaderSize size, ByteBuffer buffer) {
        return BitHeader.allocRead(size, buffer, false);
    }

    public static BitHeaderRead allocRead(HeaderSize size, ByteBuffer buffer, boolean allocOnly) {
        BitHeaderBase header = BitHeader.getHeader(size, buffer, false);
        if (!allocOnly) {
            header.read();
        }
        return header;
