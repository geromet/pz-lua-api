...
public class AnimalSynchronizationManager {
    ...
    private void sendUpdateToClient(UdpConnection connection, boolean isReliable) {
        ...
        for (short onlineID : receivedToSend) {
            ...
            long updateRate = AnimalSynchronizationManager.getUpdateRateForConnection(connection, animal);
    ...
    private static long getUpdateRateForConnection(UdpConnection connection, IsoAnimal animal) {
        ...
        if (player != null && (distance = IsoUtils.DistanceManhatten(player.getX(), player.getY(), animal.getX(), animal.getY())) <= (float)((connection.getRelevantRange() - 2) * 10) / 2.0f) {
            return -100L;
        }
        return 100L;
    }
