...
public final class ZombiePopulationManager {
                ...
                for (int x = 0; x < 8; ++x) {
                    ...
                    for (int i = 0; i < sq.getMovingObjects().size(); ++i) {
                        ...
                        if (z == 0 && sq.getRoom() == null && (realZombie.getCurrentState() == WalkTowardState.instance() || realZombie.getCurrentState() == PathFindState.instance())) {
                            ...
                            ZombiePopulationManager.n_addZombie(realZombie.getX(), realZombie.getY(), realZombie.getZ(), (byte)realZombie.dir.ordinal(), realZombie.getPersistentOutfitID(), state, realZombie.getPathTargetX(), realZombie.getPathTargetY());
                            continue;
                        }
                        DebugLog.Zombie.debugln("Virtualizing stationary Zombie: %s", realZombie);
                        ZombiePopulationManager.n_addZombie(realZombie.getX(), realZombie.getY(), realZombie.getZ(), (byte)realZombie.dir.ordinal(), realZombie.getPersistentOutfitID(), state, Integer.MIN_VALUE, Integer.MIN_VALUE);
                    }
                }
            }
        }
    ...
    public void updateMain() {
        ...
        while (offset < total) {
            ...
            int count = ZombiePopulationManager.n_getAddZombieData(offset, this.byteBuffer);
            offset += count;
    ...
    public void beginSaveRealZombies() {
        ...
        for (IsoZombie realZombie : realZombies) {
            if (realZombie.isReanimatedPlayer() || GameServer.server && realZombie.indoorZombie) continue;
        ...
        while (i < total) {
            ...
            while (i < total) {
                ...
                this.byteBuffer.put((byte)zombie.dir.ordinal());
