...
public final class VehiclePoly {
    ...
    public VehiclePoly init(BaseVehicle vehicle, float radius) {
        ...
        for (int i = 0; i < coords.length; ++i) {
            ...
            sum = (int)((float)sum + (v2.x - v1.x) * (v2.y + v1.y));
        }
        if (sum < 0) {
            Vector2 v3;
            Vector2 v1 = coords[1];
            Vector2 v2 = coords[2];
            coords[1] = v3 = coords[3];
            coords[2] = v2;
            coords[3] = v1;
        }
        this.x1 = coords[0].x;
    ...
    VehicleRect getAABB(VehicleRect rect) {
        ...
        return rect.init(null, PZMath.fastfloor(minX), PZMath.fastfloor(minY), (int)Math.ceil(maxX) - PZMath.fastfloor(minX), (int)Math.ceil(maxY) - PZMath.fastfloor(minY), PZMath.fastfloor(this.z));
    }
    ...
    public boolean containsPoint(float x, float y) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            Vector2 vector2 = v2 = i == 3 ? this.borders[0] : this.borders[i + 1];
            if (v1.y <= y) {
