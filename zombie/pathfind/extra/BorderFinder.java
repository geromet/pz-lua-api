...
public class BorderFinder {
    ...
    private final List<SquareCoord> found = new ArrayList<SquareCoord>();
    private final List<Queue<Position>> parsed = new ArrayList<Queue<Position>>();
    private final Map<SquareCoord, Position> path = new HashMap<SquareCoord, Position>();
    ...
    public BorderFinder(int x, int y, int z, int maxRange, IsoWorld world) {
        ...
        this.world = world;
        this.player = new Position(this.coordinates, Direction.NORTH, 0, 0, new EnumMap<Direction, BorderStatus>(Direction.class));
    ...
    private BorderStatus hasWall(IsoGridSquare square, Direction direction) {
        ...
        if (direction == Direction.SOUTH && ((s = this.world.currentCell.getGridSquare(c.x(), c.y(), c.z())) == null || s.getProperties().has(IsoFlagType.collideN) || s.getProperties().has(IsoFlagType.doorN))) {
            return BorderStatus.WALL;
        }
        if (direction == Direction.EAST && ((s = this.world.currentCell.getGridSquare(c.x(), c.y(), c.z())) == null || s.getProperties().has(IsoFlagType.collideW) || s.getProperties().has(IsoFlagType.doorW))) {
            return BorderStatus.WALL;
        }
        if (c.x() - this.player.coords().x() > this.maxRange || c.x() - this.player.coords().x() < -this.maxRange || c.y() - this.player.coords().y() > this.maxRange || c.y() - this.player.coords().y() < -this.maxRange) {
    ...
    private void oneStep() {
        ...
        for (Direction dir : Direction.values()) {
            ...
            if (this.found.contains(coords) || this.hasWall(square, dir) != BorderStatus.OPEN || innerSquare == null) continue;
            EnumMap<Direction, BorderStatus> walls = new EnumMap<Direction, BorderStatus>(Direction.class);
            int nwalls = 0;
