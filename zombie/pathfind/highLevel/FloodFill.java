...
public final class FloodFill {
    ...
    boolean shouldVisit(int x1, int y1, int x2, int y2) {
        ...
        FloodFill.mover.type = MoverType.Player;
        FloodFill.mover.minLevel = this.region.getLevel();
        FloodFill.mover.maxLevel = this.region.getLevel();
            ...
            for (int j = 0; j < graph.cluster.rects.size(); ++j) {
                ...
                for (int y = rect.y - 1; y < rect.y + rect.h + 1; ++y) {
                    for (int x = rect.x - 1; x < rect.x + rect.w + 1; ++x) {
                        Square square;
                        if (!chunk.contains(x, y) || (square = this.squares[this.gridX(x)][this.gridY(y)]) != null && (square.has(504) || square.hasSlopedSurface())) continue;
