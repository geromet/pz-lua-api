...
extends RandomizedBuildingBase {
    ...
    public void randomizeBuilding(BuildingDef def) {
        ...
        for (int i = 0; i < def.rooms.size(); ++i) {
            ...
            if (!this.buildingList.contains(room.name) && !room.isShop()) continue;
            choosenRoom = room;
            break;
        }
        if (choosenRoom == null) {
        ...
        for (int i = 0; i < corpseNbr; ++i) {
            IsoGridSquare freeSQ = RBShopLooted.getRandomSquareForCorpse(choosenRoom);
            RBShopLooted.createRandomDeadBody(freeSQ, null, Rand.Next(5, 10), 5, null);
        }
    }
    ...
    public boolean isValid(BuildingDef def, boolean force) {
        ...
        if (!force) {
            ...
            for (i = 0; i < GameServer.Players.size(); ++i) {
        ...
        for (i = 0; i < def.rooms.size(); ++i) {
            RoomDef room = def.rooms.get(i);
            if (!this.buildingList.contains(room.name)) continue;
            return true;
        }
