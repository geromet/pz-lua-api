            ...
            for (int y = def.y - 1; y < def.y2 + 1; ++y) {
                for (int z = -32; z < 31; ++z) {
                    ...
                    if (sq.getObjects().size() == 1) {
                        if (!Rand.NextBool(160)) continue;
                        this.addWorldItem("Dart", sq, null);
                        continue;
                    }
                    for (int i = 0; i < sq.getObjects().size(); ++i) {
                        IsoObject obj = sq.getObjects().get(i);
                        if (obj.getSprite() != null && obj.getSprite().getName() != null && (obj.getSprite().getName().equals("recreational_01_6") || obj.getSprite().getName().equals("recreational_01_7"))) {
                            if (Rand.NextBool(3)) {
                        ...
                        if (!obj.isTableSurface() || !Rand.NextBool(2)) continue;
                        if (Rand.NextBool(3)) {
