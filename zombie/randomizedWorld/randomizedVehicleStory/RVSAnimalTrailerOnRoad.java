...
extends RandomizedVehicleStoryBase {
    ...
    private static ArrayList<AnimalSpawn> getAnimalType() {
        ArrayList<AnimalSpawn> result = new ArrayList<AnimalSpawn>();
        result.add(new AnimalSpawn("bull", "cow", 1, 2));
        result.add(new AnimalSpawn("ram", "ewe", 2, 4));
        result.add(new AnimalSpawn("boar", "sow", 2, 4));
        result.add(new AnimalSpawn("cockerel", "hen", 3, 10));
        result.add(new AnimalSpawn("gobblers", "turkeyhen", 3, 6));
        result.add(new AnimalSpawn("rabdoe", "rabbuck", 3, 10));
        return result;
    }

    private static ArrayList<FoodSpawn> getFoodType() {
        ArrayList<FoodSpawn> result = new ArrayList<FoodSpawn>();
        result.add(new FoodSpawn("AnimalFeedBag", 2, 5));
        result.add(new FoodSpawn("HayTuft", 15, 50));
        result.add(new FoodSpawn("GrassTuft", 15, 50));
        return result;
    }
    ...
    public void spawnElement(VehicleStorySpawner spawner, VehicleStorySpawner.Element element) {
        ...
        switch (element.id) {
            case "vehicle1": {
                BaseVehicle vehicle1 = this.addVehicle(zone, element.position.x, element.position.y, z, element.direction, null, "Base.PickUpVan", null, "Rancher", true);
                if (vehicle1 == null) break;
                vehicle1.setAlarmed(false);
                vehicle1 = vehicle1.setSmashed("Front");
                BaseVehicle trailer = Rand.NextBool(2) ? this.addTrailer(vehicle1, zone, square.getChunk(), null, null, "Base.Trailer_Livestock") : this.addTrailer(vehicle1, zone, square.getChunk(), null, null, "Base.Trailer_Horsebox");
                spawner.setParameter("vehicle1", vehicle1);
                spawner.setParameter("trailer", trailer);
                if (Rand.Next(100) >= 80) break;
    ...
    private void spawnAnimals(VehicleStorySpawner spawner, IsoGridSquare square) {
        ArrayList<AnimalSpawn> animals = RVSAnimalTrailerOnRoad.getAnimalType();
        AnimalSpawn toSpawn = animals.get(Rand.Next(0, animals.size()));
        int nb = Rand.Next(toSpawn.minNbOfAnimals, toSpawn.maxNbOfAnimals);
        String breed = "";
        ArrayList<IsoAnimal> animalsSpawned = new ArrayList<IsoAnimal>();
        IsoAnimal male = null;
        ArrayList<IsoAnimal> female = new ArrayList<IsoAnimal>();
        ...
        for (int i = 0; i < nb; ++i) {
            ...
            breed = animal.getBreed().name;
            female.add(animal);
            if (animal.getData().getDaysSurvived() < animal.getMinAgeForBaby() || !Rand.NextBool(6)) continue;
    ...
    private void spawnFood(BaseVehicle trailer) {
        ...
        ArrayList<FoodSpawn> foodList = RVSAnimalTrailerOnRoad.getFoodType();
        FoodSpawn food = foodList.get(Rand.Next(0, foodList.size()));
