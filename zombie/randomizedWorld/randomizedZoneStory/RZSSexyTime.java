...
extends RandomizedZoneStoryBase {
    ...
    public void randomizeZoneStory(Zone zone) {
        ...
        BaseVehicle v = this.addVehicle(zone, RZSSexyTime.getSq(zone.pickedXForZoneStory, zone.pickedYForZoneStory, zone.z), null, null, "Base.VanAmbulance", null, null, null);
        if (v != null) {
            v.setAlarmed(false);
            v.setPreviouslyMoved(true);
        }
        boolean twoMale = Rand.Next(7) == 0;
        boolean bl = twoFemale = Rand.Next(7) == 0;
        if (twoMale) {
    ...
    private void addItemsOnGround(Zone zone, boolean male, BaseVehicle v) {
        ...
        if ((zeds = this.addZombiesOnVehicle(1, "Naked", maleChance, v)).isEmpty()) {
            return;
        }
