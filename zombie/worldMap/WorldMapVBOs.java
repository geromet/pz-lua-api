...
public final class WorldMapVBOs {
    ...
    private final ArrayList<WorldMapVBO> vbos = new ArrayList();
    ...
    private void flush() {
        if (this.vbos.isEmpty()) {
            WorldMapVBO vbo = new WorldMapVBO();
            vbo.create();
            this.vbos.add(vbo);
        }
        this.elements.flip();
    ...
    public void reserveVertices(int numVertices, int[] indices) {
        ...
        if ((currentCount = this.indices.position() / 2) + numVertices > 2340) {
            this.flush();
            this.addVBO();
        }
    ...
    public void drawElements(int mode, int index1, int index2, int count) {
        ...
        WorldMapVBO vbo = this.vbos.get(index1);
