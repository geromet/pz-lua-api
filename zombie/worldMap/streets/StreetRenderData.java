...
import zombie.worldMap.streets.EditStreetsV1;
...
public final class StreetRenderData {
    ...
    public void init(UIWorldMap ui, WorldMapRenderer renderer) {
        WorldMapStreet.s_charLayoutPool.releaseAll((List<CharLayout>)this.characters);
        ...
        if (ui.isMapEditor()) {
            ...
            WorldMapStreets streets;
            int i;
            boolean bEdits = false;
            for (i = 0; i < streetsAPI.getStreetDataCount(); ++i) {
                streets = streetsAPI.getStreetDataByIndex(i);
                if (!streets.checkForEdits()) continue;
                bEdits = true;
            }
        ...
        if (renderer.getBoolean("HighlightStreet")) {
            ...
            for (WorldMapStreet street : tempStreetSet) {
                if (!street.isOnScreen(ui)) continue;
    ...
    public void render(WorldMapRenderer renderer, VBORenderer vbor) {
        ...
        if (!this.triangles.isEmpty()) {
            ...
            vbor.setMode(4);
            for (int i2 = 0; i2 < this.triangles.size(); i2 += 6) {
                int n = i2;
        ...
        for (i = 0; i < this.characters.size(); ++i) {
            ...
            vbor.addQuad((float)charLayout.leftTop[0], (float)charLayout.leftTop[1], u0, v0, (float)charLayout.rightTop[0], (float)charLayout.rightTop[1], u1, v1, (float)charLayout.rightBottom[0], (float)charLayout.rightBottom[1], u2, v2, (float)charLayout.leftBottom[0], (float)charLayout.leftBottom[1], u3, v3, 0.0f, charLayout.r, charLayout.g, charLayout.b, charLayout.a);
        }
        if (bRunInProgress) {
            vbor.endRun();
        }
    ...
    boolean checkShaderUniforms(CharLayout charLayout) {
        ...
        return this.sdfOutlineR != charLayout.outlineR || this.sdfOutlineG != charLayout.outlineG || this.sdfOutlineB != charLayout.outlineB || this.sdfOutlineA != charLayout.outlineA;
    }

    void checkShaderUniforms(CharLayout charLayout, VBORenderer vbor) {
        if (charLayout.charDef.image.getTextureId() != this.textureId) {
            this.textureId = charLayout.charDef.image.getTextureId();
            vbor.setTextureID(this.textureId);
        }
        if (this.sdfThreshold != charLayout.sdfThreshold) {
            this.sdfThreshold = charLayout.sdfThreshold;
            vbor.cmdShader1f("sdfThreshold", this.sdfThreshold);
        }
        if (this.sdfShadow != charLayout.sdfShadow) {
            this.sdfShadow = charLayout.sdfShadow;
            vbor.cmdShader1f("sdfShadow", this.sdfShadow);
        }
        if (this.sdfOutlineThickness != charLayout.outlineThickness) {
            this.sdfOutlineThickness = charLayout.outlineThickness;
            vbor.cmdShader1f("sdfOutlineThick", this.sdfOutlineThickness);
        }
        if (this.sdfOutlineR != charLayout.outlineR || this.sdfOutlineG != charLayout.outlineG || this.sdfOutlineB != charLayout.outlineB || this.sdfOutlineA != charLayout.outlineA) {
            ...
            vbor.cmdShader4f("sdfOutlineColor", this.sdfOutlineR, this.sdfOutlineG, this.sdfOutlineB, this.sdfOutlineA);
        }
    }
}
