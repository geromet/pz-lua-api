...
public final class RadioDebugConsole {
        ...
        for (Map.Entry<Integer, Boolean> entry : this.state.entrySet()) {
            ...
            if (isdown && entry.getValue() != isdown) {
                switch (entry.getKey()) {
                    case 12: {
                        --this.channelIndex;
                        if (this.channelIndex >= 0 || channels == null) break;
                        this.channelIndex = channels.size() - 1;
                        break;
                    }
                    case 13: {
                        ++this.channelIndex;
                        if (channels == null || this.channelIndex < channels.size()) break;
                        this.channelIndex = 0;
                        break;
                    }
                    case 53: {
    ...
    public void render() {
        ...
        this.DrawLine(RadioDebugConsole.timeStampToString(RadioScriptManager.getInstance().getCurrentTimeStamp()), 150, true);
        this.AddBlancLine();
        this.AddBlancLine();
        ...
        if (channel != null) {
            ...
            if (broadcast != null) {
                ...
                this.DrawLine(RadioDebugConsole.timeStampToString(broadcast.getStartStamp()), 150, true);
                this.DrawLine("EndStamp: ", 0, false, this.colGrey);
                this.DrawLine(RadioDebugConsole.timeStampToString(broadcast.getEndStamp()), 150, true);
            ...
            if (script != null) {
                ...
                this.DrawLine(RadioDebugConsole.timeStampToString(script.getStartDayStamp()), 150, true);
                ...
                if (broadcast != null) {
                    ...
                    this.DrawLine(RadioDebugConsole.timeStampToString(broadcast.getStartStamp() + script.getStartDayStamp()), 150, true);
                    this.DrawLine("Real EndStamp: ", 0, false, this.colGrey);
                    this.DrawLine(RadioDebugConsole.timeStampToString(broadcast.getEndStamp() + script.getStartDayStamp()), 150, true);
