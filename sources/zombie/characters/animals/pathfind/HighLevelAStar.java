...
import zombie.characters.animals.pathfind.HighLevelGoalNode;
...
extends AStar {
    ...
    void findPath(float x1, float y1, int z1, float x2, float y2, int z2) {
        ...
        HighLevelSearchNode.pool.releaseAll((List<HighLevelSearchNode>)new ArrayList<HighLevelSearchNode>(this.nodeMap.values()));
        this.nodeMap.clear();
        ...
        if (path != null) {
            for (ISearchNode node : path) {
                HighLevelSearchNode highLevelSearchNode = (HighLevelSearchNode)node;
            }
        }
        HighLevelSearchNode.pool.releaseAll((List<HighLevelSearchNode>)new ArrayList<HighLevelSearchNode>(this.nodeMap.values()));
        this.nodeMap.clear();
