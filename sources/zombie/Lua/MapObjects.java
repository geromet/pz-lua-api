...
public final class MapObjects {
    ...
    private static Callback getOnNew(String spriteName) {
        Callback callback = onNew.get(spriteName);
        if (callback == null) {
            callback = new Callback(spriteName);
    ...
    public static void OnNewWithSprite(String spriteName, LuaClosure function, int priority) {
        ...
        Callback callback = MapObjects.getOnNew(spriteName);
        for (int i = 0; i < callback.functions.size(); ++i) {
            ...
            if (callback.priority.get(i) != priority) continue;
    ...
    public static void OnNewWithSprite(KahluaTable spriteNames, LuaClosure function, int priority) {
        ...
        while (it.advance()) {
            ...
            MapObjects.OnNewWithSprite(s, function, priority);
        }
    }
    ...
    public static void newGridSquare(IsoGridSquare square) {
        ...
        block3: for (i = 0; i < tempObjects.size(); ++i) {
            Callback callback;
            String spriteName;
            IsoObject obj = tempObjects.get(i);
            if (!square.getObjects().contains(obj) || obj instanceof IsoWorldInventoryObject || obj == null || obj.sprite == null) continue;
            String string = spriteName = obj.sprite.name == null ? obj.spriteName : obj.sprite.name;
            if (spriteName == null || spriteName.isEmpty() || (callback = onNew.get(spriteName)) == null) continue;
            MapObjects.params[0] = obj;
            for (int n = 0; n < callback.functions.size(); ++n) {
                try {
                    LuaManager.caller.protectedCallVoid(LuaManager.thread, (Object)callback.functions.get(n), params);
                }
                catch (Throwable t) {
                    ExceptionLogger.logException(t);
                }
                String string2 = spriteName = obj.sprite == null || obj.sprite.name == null ? obj.spriteName : obj.sprite.name;
                if (!square.getObjects().contains(obj) || obj.sprite == null || !callback.spriteName.equals(spriteName)) continue block3;
            }
        }
    }

    private static Callback getOnLoad(String spriteName) {
        Callback callback = onLoad.get(spriteName);
        if (callback == null) {
            callback = new Callback(spriteName);
    ...
    public static void OnLoadWithSprite(String spriteName, LuaClosure function, int priority) {
        ...
        Callback callback = MapObjects.getOnLoad(spriteName);
        for (int i = 0; i < callback.functions.size(); ++i) {
            if (callback.priority.get(i) < priority) {
                callback.functions.add(i, function);
                callback.priority.insert(i, (short)priority);
    ...
    public static void OnLoadWithSprite(KahluaTable spriteNames, LuaClosure function, int priority) {
        ...
        while (it.advance()) {
            ...
            String s = (String)value;
            MapObjects.OnLoadWithSprite(s, function, priority);
        }
    }
    ...
    public static void loadGridSquare(IsoGridSquare square) {
        ...
        block3: for (i = 0; i < tempObjects.size(); ++i) {
            ...
            if (!square.getObjects().contains(obj) || obj instanceof IsoWorldInventoryObject || obj == null || obj.sprite == null) continue;
            String string = spriteName = obj.sprite.name == null ? obj.spriteName : obj.sprite.name;
            if (spriteName == null || spriteName.isEmpty() || (callback = onLoad.get(spriteName)) == null) continue;
            MapObjects.params[0] = obj;
            for (int n = 0; n < callback.functions.size(); ++n) {
                try {
                    LuaManager.caller.protectedCallVoid(LuaManager.thread, (Object)callback.functions.get(n), params);
