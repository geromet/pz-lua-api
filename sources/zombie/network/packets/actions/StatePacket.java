...
implements INetworkPacket {
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        if (State.Stage.Exit == this.stage) {
            ...
            if (FitnessState.instance() == this.getState() && (isoGameCharacter = this.characterId.getCharacter()) instanceof IsoPlayer) {
    ...
    public void apply() {
        ...
        if (instance.isSyncOnSquare()) {
            for (Map.Entry<String, String> event : this.events.get().entrySet()) {
                boolean isClimbEvent;
                if (!StringUtils.isNullOrEmpty(event.getValue())) continue;
                boolean bl = isClimbEvent = !StringUtils.isNullOrEmpty(event.getKey()) && event.getKey().startsWith("EventClimb");
                if (PlayerActionsState.instance().equals(this.getState()) && isClimbEvent) continue;
