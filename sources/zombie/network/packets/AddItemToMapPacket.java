...
implements INetworkPacket {
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        if (this.obj.square != null) {
            DebugLog.log(DebugType.Objects, "object: added " + String.valueOf(this.obj) + " index=" + this.obj.getObjectIndex() + " " + this.obj.getX() + "," + this.obj.getY() + "," + this.obj.getZ());
            IsoObject isoObject = this.obj;
            if (isoObject instanceof IsoWorldInventoryObject) {
                IsoWorldInventoryObject worldInventoryObject = (IsoWorldInventoryObject)isoObject;
                LoggerManager.getLogger("item").write(connection.getIDStr() + " \"" + connection.getUserName() + "\" floor +1 " + this.obj.getXi() + "," + this.obj.getYi() + "," + this.obj.getZi() + " [" + worldInventoryObject.getItem().getFullType() + "]");
            } else {
                Object name;
                Object object = name = this.obj.getName() != null ? this.obj.getName() : this.obj.getObjectName();
                if (this.obj.getSprite() != null && this.obj.getSprite().getName() != null) {
                    name = (String)name + " (" + this.obj.getSprite().getName() + ")";
                }
                LoggerManager.getLogger("map").write(connection.getIDStr() + " \"" + connection.getUserName() + "\" added " + (String)name + " at " + this.obj.getX() + "," + this.obj.getY() + "," + this.obj.getZ());
            }
            this.obj.addToWorld();
            this.obj.square.RecalcProperties();
            if (!(this.obj instanceof IsoWorldInventoryObject)) {
            ...
            for (int n = 0; n < GameServer.udpEngine.connections.size(); ++n) {
                ...
                if (c.getConnectedGUID() == connection.getConnectedGUID() || !c.isRelevantTo(this.obj.square.x, this.obj.square.y)) continue;
