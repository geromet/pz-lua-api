...
final class VehicleState {
    ...
    boolean check() {
        boolean changed;
        boolean bl = changed = this.x != this.vehicle.getX() || this.y != this.vehicle.getY() || PZMath.fastfloor(this.z) != PZMath.fastfloor(this.vehicle.getZ());
        if (!changed) {
            BaseVehicle.Vector3fObjectPool pool = BaseVehicle.TL_vector3f_pool.get();
            Vector3f forward2 = this.vehicle.getForwardVector((Vector3f)pool.alloc());
            boolean bl2 = changed = this.forward.dot(forward2) < 0.999f;
            if (changed) {
