    ...
    public static boolean intersects(VehiclePoly a, VehiclePoly b) {
        for (int pn = 0; pn < 2; ++pn) {
            ...
            for (int i1 = 0; i1 < 4; ++i1) {
                int i2 = (i1 + 1) % 4;
                Vector2f p1 = PolyPolyIntersect.getPoint(polygon, i1, tempVector2f_1);
                Vector2f p2 = PolyPolyIntersect.getPoint(polygon, i2, tempVector2f_2);
                Vector2f normal = tempVector2f_3.set(p2.y - p1.y, p1.x - p2.x);
                double minA = Double.MAX_VALUE;
                double maxA = Double.NEGATIVE_INFINITY;
                for (int i = 0; i < 4; ++i) {
                    Vector2f p = PolyPolyIntersect.getPoint(a, i, tempVector2f_1);
                ...
                for (int i = 0; i < 4; ++i) {
                    Vector2f p = PolyPolyIntersect.getPoint(b, i, tempVector2f_1);
                    double projected = normal.x * p.x + normal.y * p.y;
                    if (projected < minB) {
                        minB = projected;
                    }
