...
public final class UpdateLimit {
    ...
    public UpdateLimit(long ms) {
        this.delay = ms;
        this.lastPeriod = this.last = System.currentTimeMillis();
    }

    public UpdateLimit(long ms, long shift) {
        this.delay = ms;
        this.lastPeriod = this.last = System.currentTimeMillis() - shift;
    }
    ...
    public boolean Check() {
        ...
        if (ms - this.last > this.delay) {
            this.last = ms - this.last > 3L * this.delay ? ms : (this.last += this.delay);
            return true;
        }
        return false;
    }
