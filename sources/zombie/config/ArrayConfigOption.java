...
extends ConfigOption {
    ...
    public void setValueFromObject(Object o) {
        ...
        DebugLog.log("ERROR ArrayConfigOption.setValueFromObject() \"" + this.getName() + "\" string=" + String.valueOf(o) + "\"");
    }
    ...
    public boolean isValidString(String s) {
        ...
        for (int i = 0; i < ss.length; ++i) {
            if (this.elementHandler.isValidString(ss[i])) continue;
            return false;
        }
        return true;
    }
    ...
    public ArrayConfigOption setValueVarArgs(Object ... args2) {
        int size = args2.length;
        if (this.fixedSize > 0) {
            size = PZMath.min(size, this.fixedSize);
        } else {
            this.resize(this.value, args2.length);
        }
        for (int i = 0; i < size; ++i) {
            this.value.get(i).setValueFromObject(args2[i]);
        }
        return this;
    }
    ...
    public ColorInfo getValueAsColorInfo(ColorInfo colorInfo) {
        return colorInfo.set((float)this.getElementAsDouble(0, 1.0f), (float)this.getElementAsDouble(1, 1.0f), (float)this.getElementAsDouble(2, 1.0f), (float)this.getElementAsDouble(3, 1.0f));
    }

    public double getElementAsDouble(int index, float defaultValue) {
        if (index >= 0 && index < this.value.size() && this.elementHandler instanceof DoubleConfigOption) {
            return ((DoubleConfigOption)this.getElement(index)).getValue();
        }
        return defaultValue;
    }
}
