...
public final class IsoMetaCell {
    ...
    public void checkTriggers() {
        ...
        for (int n = 0; n < this.triggers.size(); ++n) {
            ...
            if (x < tr.def.x - tr.triggerRange || x > tr.def.x2 + tr.triggerRange || y < tr.def.y - tr.triggerRange || y > tr.def.y2 + tr.triggerRange) continue;
            if (!tr.triggered) {
    ...
    public IsoMetaChunk getChunk(int x, int y) {
        if (y >= 32 || x >= 32 || x < 0 || y < 0) {
            return null;
        }
        return this.getChunk(y * 32 + x);
    }
    ...
    public void addZone(Zone zone, int cellX, int cellY) {
        ...
        for (int yy = ymin; yy <= ymax; ++yy) {
            for (int xx = xmin; xx <= xmax; ++xx) {
                if (!zone.intersects(xx * 8, yy * 8, zone.z, 8, 8)) continue;
    ...
    public void removeZone(Zone zone) {
        ...
        for (int yy = zone.y / 8; yy <= ymax; ++yy) {
            for (int xx = zone.x / 8; xx <= xmax; ++xx) {
                int i;
                if (xx < cellX / 8 || xx >= (cellX + 256) / 8 || yy < cellY / 8 || yy >= (cellY + 256) / 8 || !this.hasChunk(i = xx - cellX / 8 + (yy - cellY / 8) * 32)) continue;
    ...
    public void addRoom(RoomDef room, int cellX, int cellY) {
        ...
        for (int yy = room.y / 8; yy <= ymax; ++yy) {
            for (int xx = room.x / 8; xx <= xmax; ++xx) {
                if (xx < cellX / 8 || xx >= (cellX + 256) / 8 || yy < cellY / 8 || yy >= (cellY + 256) / 8) continue;
                int i = xx - cellX / 8 + (yy - cellY / 8) * 32;
    ...
    public void removeRooms(ArrayList<RoomDef> rooms, int userDefined) {
        for (int i = 0; i < rooms.size(); ++i) {
            RoomDef roomDef = rooms.get(i);
            if (userDefined != -1 && (userDefined != 1 || !roomDef.userDefined) && (userDefined != 0 || roomDef.userDefined)) continue;
