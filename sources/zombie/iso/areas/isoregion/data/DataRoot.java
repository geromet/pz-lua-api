...
public final class DataRoot {
    ...
    private static final WorldRegionToMetaGrid wrtmg;
    ...
    public IsoWorldRegion getIsoWorldRegion(int x, int y, int z) {
        ...
        if (this.selectInternal.chunk != null && (cr = this.selectInternal.chunk.getIsoChunkRegion(this.selectInternal.chunkSquareX, this.selectInternal.chunkSquareY, z)) != null) {
    ...
    public ArrayList<IsoWorldRegion> getIsoWorldRegionsInCell(int cellX, int cellY, ArrayList<IsoWorldRegion> worldRegions) {
        ...
        ArrayList<DataChunk> dataChunks = new ArrayList<DataChunk>();
        ...
        for (DataChunk dataChunk : dataChunks) {
            for (int z = 0; z < 32; ++z) {
                for (IsoChunkRegion chunkRegion : dataChunk.getChunkRegions(z)) {
                    ...
                    if (worldRegions.contains(worldRegion)) continue;
    ...
    public IsoChunkRegion getIsoChunkRegion(int x, int y, int z) {
        ...
        if (this.selectInternal.chunk != null) {
            return this.selectInternal.chunk.getIsoChunkRegion(this.selectInternal.chunkSquareX, this.selectInternal.chunkSquareY, z);
        }
        return null;
    }

    public void resetAllData() {
        ArrayList<IsoWorldRegion> isoWorldRegions = new ArrayList<IsoWorldRegion>();
        for (Map.Entry<Integer, DataCell> entry : this.cellMap.entrySet()) {
            DataCell cell = entry.getValue();
            for (Map.Entry<Integer, DataChunk> chunkEntry : cell.dataChunks.entrySet()) {
    ...
    public void processDirtyChunks() {
        HashSet<IsoWorldRegion> done = new HashSet<IsoWorldRegion>();
        ...
        if (!this.dirtyChunks.isEmpty()) {
            ...
            if (!this.dirtyIsoWorldRegions.isEmpty()) {
                int i2;
                for (i2 = 0; i2 < this.dirtyIsoWorldRegions.size(); ++i2) {
                    IsoWorldRegion worldRegion = this.dirtyIsoWorldRegions.get(i2);
                ...
                for (i2 = 0; i2 < this.dirtyIsoWorldRegions.size(); ++i2) {
                    ...
                    for (int j = 0; j < worldRegion.getNeighbors().size(); ++j) {
                        ...
                        if (done.contains(worldRegion1)) continue;
                        tempLocation.set(worldRegion.getCellX(), worldRegion.getCellY(), 0);
                        if (!this.changedCells.contains(tempLocation)) {
    ...
    static {
        ...
        t_end = new long[5];
        t_time = new long[5];
        wrtmg = new WorldRegionToMetaGrid();
    }
