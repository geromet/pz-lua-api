...
public class IsoGridOcclusionData {
    ...
    public boolean getCouldBeOccludedByOrphanStructures(OcclusionFilter filter) {
        ...
        if (filter == OcclusionFilter.Left) {
            return this.leftOccludedByOrphanStructures;
        }
        if (filter == OcclusionFilter.Right) {
            return this.rightOccludedByOrphanStructures;
        }
        return this.leftOccludedByOrphanStructures || this.rightOccludedByOrphanStructures;
    }

    public ArrayList<IsoBuilding> getBuildingsCouldBeOccluders(OcclusionFilter filter) {
        if (this.objectEpoch != objectEpochCount) {
            if (this.softInitialized) {
                this.Reset();
            }
        ...
        if (filter == OcclusionFilter.Right) {
            return this.rightBuildingsArray;
        }
        return this.allBuildingsArray;
    }
    ...
    private void LazyInitializeSoftOccluders() {
        ...
        this.leftOccludedByOrphanStructures |= this.GetBuildingFloorsProjectedOnSquare(_leftBuildings, x, y + 1, z);
        this.leftOccludedByOrphanStructures |= this.GetBuildingFloorsProjectedOnSquare(_leftBuildings, x + 1, y + 2, z);
        this.leftOccludedByOrphanStructures |= this.GetBuildingFloorsProjectedOnSquare(_leftBuildings, x + 2, y + 3, z);
        this.rightOccludedByOrphanStructures |= this.GetBuildingFloorsProjectedOnSquare(_rightBuildings, x + 1, y, z);
        this.rightOccludedByOrphanStructures |= this.GetBuildingFloorsProjectedOnSquare(_rightBuildings, x + 2, y + 1, z);
        this.rightOccludedByOrphanStructures |= this.GetBuildingFloorsProjectedOnSquare(_rightBuildings, x + 3, y + 2, z);
        ...
        while (z < IsoCell.maxHeight) {
            ...
            if (testSquare != null) {
                ...
                for (int dropZ = z - 1; dropZ >= 0 && building == null; --dropZ) {
                    ...
                    if (testDropSquare == null) continue;
