...
public class WeatherPeriod {
    ...
    private final ArrayList<WeatherStage> weatherStages = new ArrayList(20);
    private int weatherStageIndex;
    private final Stack<WeatherStage> stagesPool = new Stack();
    ...
    public WeatherStage getStageForWorldAge(double worldAgeHours) {
        for (int i = 0; i < this.weatherStages.size(); ++i) {
            if (!(worldAgeHours >= this.weatherStages.get(i).getStageStart()) || !(worldAgeHours < this.weatherStages.get(i).getStageEnd())) continue;
    ...
    protected void reseed(int year, int month, int day) {
        ...
        seed += (long)(month * day * 1234);
        seed += (long)((year - 1990) * month * 10000);
        this.print("Reseeding weather period, new seed: " + (seed += (long)((baseSeedB - baseSeedA) * day)));
    ...
    private void linkWeatherStages() {
        WeatherStage prev = null;
        for (int i = 0; i < this.weatherStages.size(); ++i) {
            WeatherStage curr = this.weatherStages.get(i);
            WeatherStage next = null;
    ...
    private void clearCurrentWeatherStages() {
        ...
        for (WeatherStage ws : this.weatherStages) {
            ws.reset();
    ...
    private void createWeatherPattern() {
        ...
        switch (seasonId) {
            case 5: {
                this.cloudColor = rnd < 45.0f ? this.cloudColorPurplish : this.cloudColorBlueish;
                stormChance = 10.0f;
                tropicalChance = 0.0f;
                ...
                if (!winterIsComing) break;
                blizzardChance = this.frontCache.getStrength() > 0.75f ? 100.0f : 75.0f;
                if (!(this.frontCache.getStrength() > 0.5f)) break;
                rainTimeMultiplier = 1.45f;
                break;
            }
            case 1: {
                this.cloudColor = rnd < 75.0f ? this.cloudColorGreenish : this.cloudColorBlueish;
                stormChance = 75.0f;
            ...
            case 2: {
                this.cloudColor = rnd < 25.0f ? this.cloudColorGreenish : this.cloudColorReddish;
                stormChance = 60.0f;
            ...
            case 4: {
                this.cloudColor = rnd < 50.0f ? this.cloudColorReddish : (rnd < 75.0f ? this.cloudColorPurplish : this.cloudColorBlueish);
                stormChance = 100.0f;
        ...
        this.print("WeatherPeriod: chances, storm=" + stormChance + ", tropical=" + tropicalChance + ", blizzard=" + blizzardChance + ". rainTimeMulti=" + (rainTimeMultiplier *= this.climateManager.getRainTimeMultiplierMod(SandboxOptions.instance.getRainModifier())));
        ArrayList<WeatherStage> stages = new ArrayList<WeatherStage>();
        if (this.frontCache.getType() == 1) {
            ...
            for (int i2 = 0; i2 < stages.size(); ++i2) {
                dur = (float)((double)dur + ((WeatherStage)stages.get(i2)).getStageDuration());
            }
            while (dur < maxDuration) {
                switch (this.RandNext(0, 10)) {
                    case 0: {
                        ws = this.createStage(5, 1.0f + this.RandNext(0.0f, 3.0f * this.frontCache.getStrength()));
                        break;
                    }
                    case 1: 
                    case 2: 
                    case 3: {
                        ws = this.createStage(1, 2.0f + this.RandNext(0.0f, 4.0f * this.frontCache.getStrength()));
                        break;
                    }
                    default: {
                        ws = this.createStage(2, 2.0f + this.RandNext(0.0f, 4.0f * this.frontCache.getStrength()));
                    }
                }
                dur = (float)((double)dur + ws.getStageDuration());
                stages.add(ws);
            }
        } else {
            ...
            if (this.cloudColor == this.cloudColorReddish) {
                ...
                this.cloudColor = rnd < 50.0f ? this.cloudColorBlueish : this.cloudColorPurplish;
            }
            ...
            while (dur < maxDuration) {
                switch (this.RandNext(0, 10)) {
                    case 0: {
                        ws = this.createStage(1, 2.0f + this.RandNext(0.0f, 3.0f * this.frontCache.getStrength()));
                        break;
                    }
                    case 1: 
                    case 2: 
                    case 3: 
                    case 4: {
                        ws = this.createStage(6, 2.0f + this.RandNext(0.0f, 3.0f * this.frontCache.getStrength()));
                        break;
                    }
                    default: {
                        ws = this.createStage(5, 2.0f + this.RandNext(0.0f, 3.0f * this.frontCache.getStrength()));
                    }
                }
                dur = (float)((double)dur + ws.getStageDuration());
                stages.add(ws);
        ...
        for (i = 0; i < stages.size(); ++i) {
            this.weatherStages.add((WeatherStage)stages.get(i));
            if (i >= stages.size() - 1 || !(this.RandNext(0.0f, 100.0f) < chance)) continue;
        ...
        for (i = 0; i < this.weatherStages.size(); ++i) {
            this.weatherStages.get((int)i).stageDuration *= (double)rainTimeMultiplier;
        }
        this.linkWeatherStages();
        this.duration = 0.0;
        for (i = 0; i < this.weatherStages.size(); ++i) {
        ...
        for (int i3 = 0; i3 < this.weatherStages.size(); ++i3) {
            timeWorldAgeHours = this.weatherStages.get(i3).setStageStart(timeWorldAgeHours);
        }
        this.weatherStageIndex = 0;
        this.currentStage = this.weatherStages.get(this.weatherStageIndex).startStage(this.startTime);
        this.print("WeatherPeriod: PATTERN GENERATION FINISHED.");
    ...
    public WeatherStage createAndAddModdedStage(String moddedID, double duration) {
        return this.createAndAddStage(10, duration, moddedID);
    }

    public WeatherStage createAndAddStage(int typeid, double duration) {
        return this.createAndAddStage(typeid, duration, null);
    }

    private WeatherStage createAndAddStage(int typeid, double duration, String moddedID) {
        if (this.isRunning || !this.hasStartedInit || typeid == 10 && moddedID == null) {
            return null;
        }
        WeatherStage ws = this.createStage(typeid, duration, moddedID);
        this.weatherStages.add(ws);
        return ws;
    }

    private WeatherStage createStage(int typeID, double duration) {
        return this.createStage(typeID, duration, null);
    }

    private WeatherStage createStage(int typeID, double duration, String moddedID) {
        WeatherStage stage = !this.stagesPool.isEmpty() ? this.stagesPool.pop() : new WeatherStage();
        ...
        switch (typeID) {
            case 0: {
                ...
                stage.lerpEntryTo(StrLerpVal.NextTarget);
                break;
            }
            case 1: {
                ...
                stage.lerpEntryTo(StrLerpVal.Target, StrLerpVal.NextTarget);
                break;
            }
            case 2: {
                ...
                stage.lerpEntryTo(StrLerpVal.Target, StrLerpVal.Target);
                break;
            }
            case 8: {
            ...
            case 4: {
                ...
                stage.lerpEntryTo(StrLerpVal.Target, StrLerpVal.None);
                break;
            }
            case 5: {
            ...
            case 10: {
                ...
                LuaEventManager.triggerEvent("OnInitModdedWeatherStage", this, stage, Float.valueOf(this.frontCache.getStrength()));
                break;
            }
            default: {
        ...
        switch (this.currentStage.stageId) {
            ...
            case 2: {
                ...
                if (this.currentStage.linearT < 0.1f) {
                    pushup = ClimateManager.clerp((float)((this.currentTime - this.currentStage.stageStart) / (this.currentStage.stageDuration * 0.1)), 0.0f, this.frontCache.getStrength() * 0.5f);
                } else if (this.currentStage.linearT > 0.9f) {
                    pushup = ClimateManager.clerp(1.0f - (float)((this.currentStage.stageEnd - this.currentTime) / (this.currentStage.stageDuration * 0.1)), this.frontCache.getStrength() * 0.5f, 0.0f);
                }
            ...
            case 11: {
                ...
                if (!this.currentStage.hasStartedCloud) {
                    ...
                    boolean targetRandomPlayer = (double)strength > 0.7;
                    ...
                    for (int i = 0; i < clouds; ++i) {
                        ThunderStorm.ThunderCloud cloud = this.thunderStorm.startThunderCloud(strength, angl, radius, eventFreq, thunderRatio, duration, targetRandomPlayer, this.currentStage.stageId == 11 ? kateBobStormProgress : 0.0f);
    ...
    public void update(double hoursSinceStart) {
        ...
        this.weatherNoise = 0.3f * this.frontCache.getStrength() + (float)SimplexNoise.noise(hoursSinceStart, 24000.0) * (1.0f - 0.3f * this.frontCache.getStrength());
        ...
        this.temperatureInfluence = this.frontCache.getType() == 1 ? this.climateManager.temperature.internalValue - temperatureMod : this.climateManager.temperature.internalValue + temperatureMod;
        if (this.isRunning) {
    ...
    public void save(DataOutputStream output) throws IOException {
        if (!GameClient.client || GameServer.server) {
            ...
            if (this.isRunning) {
                ...
                for (int i = 0; i < this.weatherStages.size(); ++i) {
                    WeatherStage ws = this.weatherStages.get(i);
        ...
        if (hasstuff == 1) {
            ...
            if (this.isRunning) {
                ...
                for (i = 0; i < stagesCount; ++i) {
                    ...
                    WeatherStage ws = !this.stagesPool.isEmpty() ? this.stagesPool.pop() : new WeatherStage();
                    ws.stageId = id;
    ...
    public static class WeatherStage {
        ...
        public void load(DataInputStream input, int worldVersion) throws IOException {
            ...
            this.lerpMidVal = StrLerpVal.fromValue(input.readInt());
            this.lerpEndVal = StrLerpVal.fromValue(input.readInt());
        ...
        protected WeatherStage startStage(double exitTime) {
        ...
        protected WeatherStage setStageDuration(double time) {
        ...
        protected WeatherStage overrideStageDuration(double time) {
            this.stageDuration = time;
            return this;
        }
        ...
        public void lerpEntryTo(int mid, int end) {
            if (!this.creationFinished) {
                this.lerpEntryTo(StrLerpVal.fromValue(mid), StrLerpVal.fromValue(end));
            }
        }

        protected void lerpEntryTo(StrLerpVal end) {
            this.lerpEntryTo(StrLerpVal.None, end);
        }

        protected void lerpEntryTo(StrLerpVal mid, StrLerpVal end) {
            if (!this.creationFinished) {
                this.lerpMidVal = mid;
                this.lerpEndVal = end;
            }
        ...
        private float getLerpValue(StrLerpVal lerpVal) {
            switch (lerpVal.ordinal()) {
                case 0: {
        ...
        private WeatherStage updateT(double hour) {
        ...
        private float getPeriodLerpT(double hour) {
            ...
            if (hour > this.stageEnd) {
                return 1.0f;
            }
            return (float)((hour - this.stageStart) / this.stageDuration);
        }
    }
    ...
    public static enum StrLerpVal {
        ...
        public static StrLerpVal fromValue(int id) {
