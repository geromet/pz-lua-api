...
public final class IsoDirectionFrame {
    ...
    void render(ObjectRenderEffects dr, float sx, float sy, float width, float height, IsoDirections dir, ColorInfo info, boolean flip, Consumer<TextureDraw> texdModifier) {
        Texture tex = this.directions[dir.ordinal()];
        if (tex == null) {
            return;
        }
    ...
    public void renderexplicit(int sx, int sy, IsoDirections dir, float scale, ColorInfo color) {
        ...
        tex.renderstrip(sx, sy, (int)((float)tex.getWidth() * scale), (int)((float)tex.getHeight() * scale), r, g, b, a, null);
    }

    public boolean hasNoTextures() {
        for (int i = 0; i < this.directions.length; ++i) {
            if (this.directions[i] == null) continue;
            return false;
        }
        return true;
    }
