...
public final class FBORenderChunkManager {
    ...
    public void recycle() {
        for (int i = 0; i < this.toRecycle.size(); ++i) {
            ArrayList<Object> st;
            FBORenderChunk fboRenderChunk = (FBORenderChunk)this.toRecycle.get(i);
            int sizeIndex = fboRenderChunk.h;
            if (fboRenderChunk.submitted) continue;
    ...
    public void submitCachesForFrame() {
        FBORenderChunk renderChunk1;
        int i;
        int playerIndex = IsoCamera.frameState.playerIndex;
        TIntObjectHashMap cachedRenderChunkIndexMap = SpriteRenderer.instance.getPopulatingState().cachedRenderChunkIndexMap;
        this.tempRenderChunks.clear();
        cachedRenderChunkIndexMap.forEachValue(fboRenderChunk -> {
            this.tempRenderChunks.add((FBORenderChunk)fboRenderChunk);
            return true;
        });
        for (i = 0; i < this.tempRenderChunks.size(); ++i) {
            renderChunk1 = this.tempRenderChunks.get(i);
            if (renderChunk1.getRenderLevels().getPlayerIndex() != playerIndex) continue;
            cachedRenderChunkIndexMap.remove(renderChunk1.index);
        }
        for (i = 0; i < this.toRenderThisFrame.size(); ++i) {
            renderChunk1 = this.toRenderThisFrame.get(i);
    ...
    public boolean beginRenderChunkLevel(IsoChunk chunk, int level, float cameraZoom, boolean canRender, boolean canClear) {
        ...
        this.xoff = (float)textureWidth / 2.0f;
    ...
    public void renderThreadChunkStart(int index, boolean bClear) {
        this.renderThreadCurrent = (FBORenderChunk)SpriteRenderer.instance.getRenderingState().cachedRenderChunkIndexMap.get(index);
        if (this.renderThreadCurrent == null) {
            return;
        }
    ...
    private void checkCombinedFBO() {
        int width = (int)Math.ceil((float)Core.width * 2.5f);
        int height = (int)Math.ceil((float)Core.height * 2.5f);
    ...
    public void endFrame() {
        ...
        if (DebugOptions.instance.fboRenderChunk.combinedFbo.getValue() && !this.toRenderThisFrame.isEmpty()) {
            ...
            offscreenWidth = Core.getInstance().getOffscreenWidth(playerIndex);
            offscreenHeight = Core.getInstance().getOffscreenHeight(playerIndex);
            SpriteRenderer.instance.glDoEndFrame();
            ...
            this.combinedTexture.rendershader2(0.0f + camera.fixJigglyModelsX, 0.0f + camera.fixJigglyModelsY, Core.width, Core.height, 0, this.combinedTexture.getHeight() - (int)((float)Core.height * zoom), (int)((float)Core.width * zoom), (int)((float)Core.height * zoom), 1.0f, 1.0f, 1.0f, 1.0f);
            if (SceneShaderStore.chunkRenderShader != null) {
                IndieGL.EndShader();
            }
            SpriteRenderer.instance.glDoEndFrame();
    ...
    public void Reset() {
        ...
        for (ArrayList<FBORenderChunk> renderChunks : this.sizeChunkStore.values()) {
            for (FBORenderChunk renderChunk : renderChunks) {
                if (renderChunk.fbo == null) continue;
