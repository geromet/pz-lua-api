...
public abstract class DictionaryInfo<T extends DictionaryInfo<T>> {
    ...
    public String ToString() {
        return "TYPE = " + this.getInfoType() + ",registryID = " + this.registryId + ",fulltype = \"" + this.fullType + "\",modID = \"" + this.modId + "\",existsAsVanilla = " + this.existsAsVanilla + ",isModded = " + this.isModded + ",obsolete = " + this.obsolete + ",removed = " + this.removed + ",modOverrides = " + (this.modOverrides != null ? this.modOverrides.size() : 0) + ",";
    }
    ...
    protected void saveAsText(FileWriter w, String padding) throws IOException {
        ...
        if (this.modOverrides != null) {
            Object txt = "modOverrides = { ";
            for (int i = 0; i < this.modOverrides.size(); ++i) {
                txt = (String)txt + "\"" + this.modOverrides.get(i) + "\"";
                if (i >= this.modOverrides.size() - 1) continue;
                txt = (String)txt + ", ";
            }
            txt = (String)txt + " },";
            w.write(padding + (String)txt + System.lineSeparator());
        }
    }
    ...
    protected void load(ByteBuffer bb, int version, List<String> modIDs, List<String> modules) {
        ...
        this.moduleName = modules.get(modules.size() > 127 ? bb.getShort() : (short)bb.get());
        this.name = GameWindow.ReadString(bb);
        this.fullType = this.moduleName + "." + this.name;
        byte bits = bb.get();
        if (Bits.hasFlags(bits, 1)) {
            this.modId = modIDs.get(modIDs.size() > 127 ? bb.getShort() : (short)bb.get());
            this.isModded = true;
        } else {
        ...
        if (Bits.hasFlags(bits, 16)) {
            ...
            if (!Bits.hasFlags(bits, 32)) {
                this.modOverrides.add(modIDs.get(modIDs.size() > 127 ? bb.getShort() : (short)bb.get()));
            } else {
                int count = bb.get();
