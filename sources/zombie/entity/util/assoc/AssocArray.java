...
public class AssocArray<K, V> {
    ...
    private Object[] grow(int minCapacity) {
        ...
        this.elementData = new Object[Math.max(10, minCapacity)];
        return this.elementData;
    }
    ...
    private V setInternal(int frontIndex, V v) {
        ...
        this.elementData[this.realValueIndex((int)frontIndex)] = v;
        return oldValue;
    }
    ...
    private void add(K k, V v, Object[] elementData, int s) {
        ...
        elementData[this.realKeyIndex((int)s)] = k;
        elementData[this.realValueIndex((int)s)] = v;
        this.size = s + 1;
    }
    ...
    public V removeIndex(int frontIndex) {
        Objects.checkIndex(frontIndex, this.size);
        Object[] es = this.elementData;
        Object oldValue = es[this.realValueIndex(frontIndex)];
    ...
    public void addAll(AssocArray<K, V> other) {
        for (int i = 0; i < other.size; ++i) {
            K otherK = other.getKey(i);
            V otherV = other.getValue(i);
            if (this.containsKey(otherK)) continue;
            this.add(otherK, otherV);
        }
    }
    ...
    public void forEach(BiConsumer<? super K, ? super V> action) {
        ...
        for (int i = 0; this.modCount == expectedModCount && i < size; ++i) {
            action.accept(AssocArray.keyAt(es, i), AssocArray.valueAt(es, i));
        }
        if (this.modCount != expectedModCount) {
            throw new ConcurrentModificationException();
        }
    ...
    protected void debugPrint() {
        ...
        for (int i = 0; i < this.size; ++i) {
            DebugLog.log("[" + i + "][" + this.realKeyIndex(i) + "] " + String.valueOf(this.elementData[this.realKeyIndex(i)]));
            DebugLog.log("   [" + this.realValueIndex(i) + "] " + String.valueOf(this.elementData[this.realValueIndex(i)]));
        }
    }
}
