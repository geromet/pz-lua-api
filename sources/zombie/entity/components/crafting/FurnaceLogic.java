        ...
        private void load(ByteBuffer input, int worldVersion) throws IOException {
            if (input.get() != 0) {
                ...
                if (this.currentRecipe == null || scriptVersion != this.currentRecipe.getScriptVersion()) {
                    this.currentRecipe = null;
                    this.elapsedTime = 0;
                    DebugLog.General.warn("FurnaceSlot[" + this.index + "] CraftRecipe '" + recipeName + "' is null (" + (this.currentRecipe == null) + ", or has script version mismatch. Cancelling current craft.");
                }
            }
