...
public class ResourceBlueprint {
    ...
    public static ResourceBlueprint alloc(String id, ResourceType type, ResourceIO io, float capacity, String filter, ResourceChannel channel, EnumBitStore<ResourceFlag> flags) {
        ResourceBlueprint bp = ResourceBlueprint.alloc_empty();
    ...
    private static String checkCharacters(String s) {
        if (Core.debug && (s.contains(serialElementSeparator) || s.contains(serialSubSeparator))) {
    ...
    public static String Serialize(ResourceBlueprint bp) {
        return ResourceBlueprint.Serialize(bp.id, bp.type, bp.io, bp.capacity, bp.stackAnyItem, bp.filter, bp.channel, bp.resourceFlags);
    }
    ...
    public static String Serialize(String id, ResourceType type, ResourceIO io, float capacity, boolean stackAnyItem, String filter, ResourceChannel channel, EnumBitStore<ResourceFlag> flags) {
        ...
        sb.setLength(0);
        sb.append(ResourceBlueprint.checkCharacters(id));
        sb.append(serialElementSeparator);
        ...
        if (stackAnyItem) {
            sb.append(serialElementSeparator);
            sb.append(stackAnyItemIdentifier);
        }
        if (channel != null && channel != ResourceChannel.NO_CHANNEL || flags != null && !flags.isEmpty() || !StringUtils.isNullOrWhitespace(filter)) {
            ...
            if (!StringUtils.isNullOrWhitespace(filter)) {
                sb.append(ResourceBlueprint.checkCharacters(filter));
            } else {
                sb.append(str_null);
            }
            sb.append(serialElementSeparator);
    ...
    public static ResourceBlueprint Deserialize(ResourceBlueprint bp, String serial, boolean flagsAsString) {
        ...
        bp.capacity = bp.type == ResourceType.Item ? (float)Integer.parseInt(elements[3]) : Float.parseFloat(elements[3]);
        if (elements.length == 5 && elements[4].equalsIgnoreCase(stackAnyItemIdentifier)) {
        ...
        if (elements.length == 7) {
            ...
            if (!str_null.equalsIgnoreCase(filterStr)) {
                bp.filter = filterStr;
            }
            if (!str_null.equalsIgnoreCase(channelStr = elements[5])) {
                bp.channel = ResourceChannel.valueOf(channelStr);
            }
            if (!str_null.equalsIgnoreCase(flagsStr = elements[6])) {
                if (flagsAsString) {
                    String[] flags = flagsStr.split(serialSubSeparator);
