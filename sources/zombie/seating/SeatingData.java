...
public final class SeatingData {
    ...
    public SeatingFile.Position getPositionWithID(String tilesetName, int col, int row, String id) {
        ...
        for (SeatingFile.Position position : tile.positions) {
            if (!position.id.equalsIgnoreCase(id)) continue;
            return position;
        }
        return null;
    }
    ...
    public String getPositionProperty(String tilesetName, int col, int row, int index, String key) {
        ...
        return position.properties.get(key);
    }
        ...
        for (SeatingFile.Tileset tileset : this.file.tilesets) {
            for (SeatingFile.Tile tile : tileset.tiles) {
                for (SeatingFile.Position position : tile.positions) {
                    ...
                    String spriteName;
                    IsoSprite sprite;
                    if (!"default".equalsIgnoreCase(position.id) || (sprite = IsoSpriteManager.instance.namedMap.get(spriteName = String.format("%s_%d", tileset.name, tile.col + tile.row * 8))) == null || (facing = sprite.getProperties().get(IsoPropertyType.FACING)) == null) continue;
                    position.id = facing;
                }
            }
        }
