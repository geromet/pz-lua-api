...
public final class TextLayout {
    ...
    public TextLayout splitLines(String text1) {
        ...
        for (int i = 0; i < this.textWithoutFormatting.length(); ++i) {
            ...
            if (ch == '\n') {
                this.addLine((short)lineStart, (short)(i - 1));
                lineEnd = lineStart = (short)(i + 1);
                continue;
            }
            lineEnd = (short)i;
        }
        if (lineStart < this.textWithoutFormatting.length()) {
            this.addLine((short)lineStart, (short)lineEnd);
        }
        return this;
    }

    public void addLine(short start, short end) {
        this.numLines = (short)(this.numLines + 1);
    ...
    public float getLineOffsetX(int lineIndex) {
        return (float)(this.getMaxLineLength() - this.getLineLength(lineIndex)) / 2.0f;
    }
